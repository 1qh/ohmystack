...
Playwright coverage is organized as product-level browser tests around `agent`, with shared E2E support code separated into reusable test infrastructure layers.
...
- Playwright runs against `agent` with test-mode backend identity and deterministic environment setup.
...
```mermaid
flowchart LR
PW[Playwright] --> NX[Next.js agent]
  NX --> CVX[Convex backend/agent]
...
cd backend/agent && bun with-env convex dev --once
# Run E2E
cd agent
...
Backend tests go in `backend/agent/convex/f.test.ts`. Follow the existing pattern:
...
4. Use `ctx.run(async c => c.db.query(...))` to inspect DB state E2E tests go in `agent/e2e/<category>.test.ts`. Follow the fixture pattern:
5. Import `{ test, expect }` from `./fixtures`
6. Use page objects (`sessionListPage`, `chatPage`) for locators
7. Use `.is-user, .is-assistant` selector for message elements (ai-elements DOM)

### Test File Layout

```
backend/agent/convex/f.test.ts        — 858 backend tests
backend/agent/convex/prod-smoke.test.ts — 1 production smoke test
agent/e2e/session.test.ts             — session management E2E
agent/e2e/chat.test.ts                — chat & streaming E2E
agent/e2e/settings.test.ts            — MCP settings E2E
agent/e2e/error.test.ts               — error states E2E
agent/e2e/a11y.test.ts                — accessibility E2E
agent/e2e/frontend-states.test.ts     — frontend states E2E
agent/e2e/real-world.test.ts          — real-world scenario E2E
```
