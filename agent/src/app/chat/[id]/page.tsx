...
const parseDelegateTaskId = (result?: string): Id<'tasks'> | null => {
...
const WorkerStreamPanel = ({ taskId }: { taskId: Id<'tasks'> }) => {
  ...
  const workerMessages = useQuery(api.messages.listMessages, task?.threadId ? { threadId: task.threadId } : 'skip')
  if (!task) return null
  const statusClass =
...
const ChatMessageRow = ({ message }: { message: ChatMessageData }) => {
  ...
  const isAssistantStreaming = !message.isComplete && message.role === 'assistant'
  const textContent = message.isComplete ? message.content : (message.streamingContent ?? message.content)
  const { reasoningParts, sourceParts, toolParts } = splitMessageParts({ parts })
  return (
...
const ChatPage = () => {
  ...
  const id = params.id as Id<'session'>
  const tokenUsageQuery = (api as { tokenUsage: { getTokenUsage: unknown } }).tokenUsage.getTokenUsage
  const session = useQuery(api.sessions.getSession, { sessionId: id })
  const messages = useQuery(api.messages.listMessages, session ? { threadId: session.threadId } : 'skip') as
    | ChatMessageData[]
    | undefined
  const tasks = useQuery(api.tasks.listTasks, { sessionId: id }) as SessionTask[] | undefined
  const todos = useQuery(api.todos.listTodos, { sessionId: id }) as SessionTodo[] | undefined
  const tokenUsage = useQuery(tokenUsageQuery as never, { sessionId: id } as never) as SessionTokenUsage | undefined
  const submitMessage = useMutation(api.orchestrator.submitMessage)
  const lastMessage = messages && messages.length > 0 ? (messages.at(-1) ?? null) : null
  const isTyping = lastMessage?.role === 'assistant' && !lastMessage.isComplete
  const submitChatMessage = async ({ content, sessionId }: { content: string; sessionId: Id<'session'> }) => {
    setSending(true)
    try {
  ...
  const runSubmitChatMessage = ({ content, sessionId }: { content: string; sessionId: Id<'session'> }) => {
