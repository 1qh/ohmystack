# Frontend Plan (`apps/agent`)

## Scope and Constraints

- Build a standalone Next.js App Router frontend at `apps/agent/` with its own Convex client and `NEXT_PUBLIC_CONVEX_URL`.
- Do not reuse demo-app Convex provider wiring from `apps/convex/*`; this app targets `packages/be-agent` only.
