...
const resolveSessionByThreadId = async ({ ctx, threadId }: { ctx: Pick<QueryCtx, 'db'>; threadId: string }) =>
  ctx.db
    .query('session')
    .withIndex('by_threadId', idx => idx.eq('threadId', threadId))
    .unique()
const hasTerminalToolParts = ({ parts }: { parts: { status?: 'error' | 'pending' | 'success'; type: string }[] }) => {
  for (const p of parts) if (p.type === 'tool-call' && !(p.status === 'success' || p.status === 'error')) return false
  return true
}
const readRunStateByThreadId = async ({ ctx, threadId }: { ctx: Pick<QueryCtx, 'db'>; threadId: string }) =>
  ctx.db
    .query('threadRunState')
    .withIndex('by_threadId', idx => idx.eq('threadId', threadId))
    .unique()
const getContextSizeInline = async ({ ctx, threadId }: { ctx: Pick<QueryCtx, 'db'>; threadId: string }) => {
  ...
  const rows = await ctx.db
    ...
    .order('desc')
    .take(SCAN_LIMIT + 1)
  const hasMore = rows.length > SCAN_LIMIT
  const selected = rows.slice(0, SCAN_LIMIT)
...
const acquireCompactionLockInline = async ({ ctx, threadId }: { ctx: Pick<MutationCtx, 'db'>; threadId: string }) => {
  const runState = await readRunStateByThreadId({ ctx, threadId })
  if (!runState) return { lockToken: '', ok: false }
  const now = Date.now()
  const lockExpired = runState.compactionLockAt !== undefined && now - runState.compactionLockAt > LOCK_TTL_MS
  const lockOpen = !(runState.compactionLock && runState.compactionLockAt) || lockExpired
...
const listClosedPrefixGroupsInline = async ({ ctx, threadId }: { ctx: Pick<QueryCtx, 'db'>; threadId: string }) => {
  const runState = await readRunStateByThreadId({ ctx, threadId })
  const rows = await ctx.db
    .query('messages')
    .withIndex('by_threadId', idx => idx.eq('threadId', threadId))
...
const releaseCompactionLockInline = async ({
  ctx,
  lockToken,
  threadId
}: {
...
const snapshotTodosInline = async ({ ctx, threadId }: { ctx: Pick<MutationCtx, 'db'>; threadId: string }) => {
...
const restoreTodosIfMissingInline = async ({
  ctx,
  snapshot,
  threadId
}: {
...
const snapshotTodos = internalMutation({
  args: { threadId: v.string() },
  handler: async (ctx, { threadId }) => snapshotTodosInline({ ctx, threadId })
})
const restoreTodosIfMissing = internalMutation({
  args: {
    snapshot: v.array(
      v.object({
        content: v.string(),
...
const compactIfNeeded = internalMutation({
  args: { threadId: v.string() },
  handler: async (ctx, { threadId }) => {
    const contextSize = await getContextSizeInline({ ctx, threadId })
    const overThreshold = contextSize.charCount > CHAR_THRESHOLD || contextSize.messageCount > MESSAGE_THRESHOLD
