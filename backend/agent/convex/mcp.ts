...
const getNameFromUnknown = ({ value }: { value: unknown }) => {
...
const parseCachedToolNames = ({ cachedTools }: { cachedTools?: string }) => {
...
const parseJsonObject = ({ raw }: { raw: string }) => {
  const parsed = JSON.parse(raw) as unknown
  if (!parsed || Array.isArray(parsed) || typeof parsed !== 'object') throw new Error('invalid_json_object')
  return parsed as Record<string, unknown>
}
const withMcpTimeout = async <T>({ operation, promise }: { operation: string; promise: Promise<T> }) =>
...
const redactServer = (doc: null | Record<string, unknown>) => {
  if (!doc) return null
...
const { create, rm, update } = crud('mcpServers', owned.mcpServer, { hooks })
const read = q({
...
const mcpDiscover = internalMutation({
...
const mcpCallTool = internalMutation({
  args: {
    serverName: v.string(),
  ...
  handler: async (ctx, { serverName, sessionId, threadId, toolArgs, toolName }) => {
    ...
    const firstHit = toolInCache({ allowStale: false, row: server })
    if (!firstHit) {
