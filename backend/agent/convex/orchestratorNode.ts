...
const finishRunRef = makeFunctionReference<'mutation', { runToken: string; threadId: string }, { scheduled: boolean }>(
  'orchestrator:finishRun'
)
const heartbeatRunRef = makeFunctionReference<'mutation', { runToken: string; threadId: string }, undefined>(
  'orchestrator:heartbeatRun'
)
const recordRunErrorRef = makeFunctionReference<'mutation', { error: string; threadId: string }, undefined>(
  'orchestrator:recordRunError'
)
const readRunStateRef = makeFunctionReference<'query', { threadId: string }, Doc<'threadRunState'> | null>(
  'orchestrator:readRunState'
)
const readSessionByThreadRef = makeFunctionReference<'query', { threadId: string }, Doc<'session'> | null>(
...
const postTurnAuditFencedRef = makeFunctionReference<
  'mutation',
...
const incrementTaskToolCounterRef = makeFunctionReference<
  'mutation',
  { threadId: string; toolName: string },
  { shouldRemind: boolean; turnsSinceTaskTool: number }
>('orchestrator:incrementTaskToolCounter')
const consumeTaskReminderRef = makeFunctionReference<'mutation', { threadId: string }, { shouldInject: boolean }>(
  'orchestrator:consumeTaskReminder'
)
const listActiveTasksByThreadRef = makeFunctionReference<'query', { threadId: string }, Doc<'tasks'>[]>(
...
const buildTaskReminder = ({ tasks }: { tasks: Doc<'tasks'>[] }) => {
  const lines = [
    '<system-reminder>',
    '[TASK CHECK REMINDER]',
...
const isRunStale = async ({ ctx, runToken, threadId }: { ctx: ActionCtx; runToken: string; threadId: string }) => {
  const state = await ctx.runQuery(readRunStateRef, { threadId })
  if (!state) return true
  if (state.status !== 'active') return true
  return state.activeRunToken !== runToken
}
const startHeartbeat = ({ ctx, runToken, threadId }: { ctx: ActionCtx; runToken: string; threadId: string }) =>
  setInterval(
    () => {
...
const buildSystemPromptWithReminder = async ({ ctx, threadId }: { ctx: ActionCtx; threadId: string }) => {
...
const consumeResultStream = async ({
  ctx,
  messageId,
  result
}: {
...
const runOrchestrator = internalAction({
  ...
  handler: async (ctx, { promptMessageId, runToken, threadId }) => {
    ...
    try {
      ...
      const { collectedParts, fullText } = await consumeResultStream({
        ctx,
        messageId,
        result
      })
