...
const IDLE_AFTER_MS = DAY_MS
const RETENTION_MS = 180 * DAY_MS
const MAX_BATCH = 100
const MAX_SESSIONS_PER_CLEANUP = 10
const archiveIdleSessions = internalMutation({
  args: {},
  handler: async ctx => {
    const now = Date.now()
    const idleBefore = now - IDLE_AFTER_MS
    const archiveBefore = now - ARCHIVE_AFTER_MS
...
const cleanupArchivedSessions = internalMutation({
  ...
  handler: async ctx => {
    const now = Date.now()
    const deleteBefore = now - RETENTION_MS
