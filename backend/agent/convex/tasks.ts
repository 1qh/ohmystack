...
const buildTaskCompletionReminder = ({ description, taskId }: { description: string; taskId: string }) =>
  [
    '<system-reminder>',
...
const buildTaskTerminalReminder = ({
  description,
  error,
  status,
  taskId
...
const maybeContinueOrchestratorInline = async ({ ctx, taskId }: { ctx: Pick<MutationCtx, 'db'>; taskId: Id<'tasks'> }) => {
...
const updateTaskHeartbeat = internalMutation({
  args: { taskId: v.id('tasks') },
  handler: async (ctx, { taskId }) => {
    const task = await ctx.db.get(taskId)
    if (!task) return
...
const maybeContinueOrchestrator = internalMutation({
  args: { taskId: v.id('tasks') },
