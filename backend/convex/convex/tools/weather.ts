...
const weatherResultSchema = v.object({
  city: v.string(),
...
const fetchWeather = async (city: string, unit: 'celsius' | 'fahrenheit' = 'celsius') => {
  ...
  const geoData = geoApiResponseSchema.safeParse(geoJson)
  ...
  const weatherData = weatherApiResponseSchema.safeParse(weatherJson)
