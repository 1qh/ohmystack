...
const extractLastUserMessage = (prompt: LanguageModelV3Prompt): string => {
...
const extractCityFromPrompt = (userMsg: string): string => {
...
const createWeatherToolCall = (city: string): LanguageModelV3Content => ({
...
const createGenerateResult = (prompt: LanguageModelV3Prompt): LanguageModelV3GenerateResult => {
  const { content, finishReason } = getResponseForPrompt(prompt)
  return { content, finishReason, usage: mockUsage, warnings: [] }
}
const createStreamResult = (prompt: LanguageModelV3Prompt): LanguageModelV3StreamResult => {
  const { content, finishReason } = getResponseForPrompt(prompt)
