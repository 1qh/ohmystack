...
**16 hooks** (parameter list before generic constraints):
...
Scanned **303 files**, **6967 declared symbols**. No `@beta`/`@alpha`/`@experimental`/`@deprecated`/`@internal` JSDoc tags found — entire public surface is **stable**.

To mark API stability, add JSDoc tags above an export:

```ts
/** @beta New shape, may change without major version bump. */
export const someExperiment = ...
```
...
Coverage of public exports (every name reachable through `noboil/...` subpaths) by mention in `doc/content/docs/*.mdx`. **381/381 mentioned (100%).**
