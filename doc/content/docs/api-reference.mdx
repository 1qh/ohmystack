...
Scanned **337 files**, **7672 declared symbols**. No `@beta`/`@alpha`/`@experimental`/`@deprecated`/`@internal` JSDoc tags found — entire public surface is **stable**.
...
**40 subpath exports** from `noboil` package.json. Use exact import paths for tree-shaking.
...
| `noboil/convex/tools`              | `./src/convex/tools/_lib/index.ts`          | —           |
...
Coverage of public exports (every name reachable through `noboil/...` subpaths) by mention in `doc/content/docs/*.mdx`. **469/469 mentioned (100%).**
