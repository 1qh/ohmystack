...
|---|---|---|
| `log` | `append_{table}` | Atomic seq + insert. Idempotent if `idempotency_key` provided. |
|   | `bulk_append_{table}` | Single-transaction batch insert. |
|   | `purge_{table}_by_parent` | Soft- or hard-delete all rows for a parent. |
|   | `restore_{table}_by_parent` | Bring back soft-deleted rows for a parent. Requires `softDelete: true`. |
|   | `rm_{table}` | Hard-delete a single row by id. Author-only. |
|   | `bulk_rm_{table}` | Hard-delete multiple rows. Author-only. |
|   | `update_{table}` | Update a row's payload (preserves seq + parent). Author-only. |
| `kv` | `set_{table}` | Upsert with optional `expectedUpdatedAt` conflict check. |
|   | `rm_{table}` | Soft- or hard-delete by key. |
|   | `restore_{table}` | Bring back a soft-deleted key. Requires `softDelete: true`. |
| `quota` | `consume_{table}` | Atomic check + record. Throws when over limit. |
|   | `record_{table}` | Append timestamp + prune expired. Always succeeds. |
