...
| `log`       | 390 / 256          | 71 / 73             | 11 / 10          | 357L     | 5 / 5 ✓         |
...
**238 shared files · 47 cvx-only · 35 stdb-only · 1 files with cross-backend symbol divergence.** Status: 🔴 5 unaccounted-for gap(s).

| Pair                     | shared    | cvx-only           | stdb-only          | symbol gaps | status |
| ------------------------ | --------: | ------------------ | ------------------ | ----------: | ------ |
| **lib/noboil/src**       | 78        | 8 (4 unaccounted)  | 22 (0 unaccounted) | 1           | 🔴     |
...
- **lib/noboil/src**: cvx-only file `server/budget.ts`, `server/audit.ts`, `__tests__/audit.test.ts`, `__tests__/budget.test.ts`
