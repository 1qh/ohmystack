...
**539/541 blocks parseable (100%).** Snippets without TypeScript-shaped syntax (config JSON, shell, mermaid) are skipped — they're counted as parseable but not actually checked.
...
| `log`       | 390 / 256          | 71 / 73             | 12 / 10          | 357L     | 5 / 5 ✓         |
...
| `index.ts`             | ✓ 11L  | ✓ 12L  | 20             | —        | —         | 🟢     |
| `misc.tsx`             | ✓ 11L  | ✓ 15L  | 3              | —        | —         | 🟢     |
| `permission-guard.tsx` | ✓ 24L  | ✓ 24L  | 1              | —        | —         | 🟢     |
...
**238 shared files · 79 cvx-only · 35 stdb-only · 1 files with cross-backend symbol divergence.** Status: 🔴 37 unaccounted-for gap(s).

| Pair                     | shared    | cvx-only            | stdb-only          | symbol gaps | status |
| ------------------------ | --------: | ------------------- | ------------------ | ----------: | ------ |
| **lib/noboil/src**       | 78        | 40 (36 unaccounted) | 22 (0 unaccounted) | 1           | 🔴     |
...
| **web/movie**            | 16        | 1 (0 unaccounted)   | 1 (0 unaccounted)  | 0           | 🟢     |
...
- **lib/noboil/src**: cvx-only file `tools/_lib/caller-runtime.ts`, `tools/_lib/define-provider.ts`, `tools/_lib/prompt-blocks.ts`, `tools/_lib/http.ts`, `tools/_lib/validate.ts`, `tools/_lib/builder.ts`, `tools/_lib/parser.ts`, `tools/_lib/codegen/schema.ts`, `tools/_lib/codegen/emit.ts`, `tools/_lib/codegen/scan.ts`, `tools/_lib/codegen/extract-meta.ts`, `tools/_lib/codegen/index.ts`, `tools/_lib/types.ts`, `tools/_lib/__tests__/dispatch.test.ts`, `tools/_lib/__tests__/manifest.test.ts`, `tools/_lib/__tests__/parser.test.ts`, `tools/_lib/__tests__/boundary.test.ts`, `tools/_lib/__tests__/step-sink.test.ts`, `tools/_lib/__tests__/to-dispatch-error.test.ts`, `tools/_lib/__tests__/error.test.ts`, `tools/_lib/manifest.ts`, `tools/_lib/hermetic.ts`, `tools/_lib/index.ts`, `tools/_lib/error.ts`, `tools/_bin/new-tool.ts`, `tools/_bin/x-codegen.ts`, `tools/_bin/remove-tool.ts`, `tools/_bin/x.ts`, `tools/_bin/x-docgen.ts`, `server/budget.ts`, `server/test-harness.ts`, `server/audit.ts`, `__tests__/budget.property.test.ts`, `__tests__/audit.test.ts`, `__tests__/budget.test.ts`, `__tests__/budget.synthetic.test.ts`
- `lib/noboil/src/server/index.ts` — cvx-only: `createTestHarness`, `makeAudit`, `makeBudget`, `periodKeyFor` · stdb-only: —
