...
Walks every `*.ts*` in `lib/noboil/src/{convex,spacetimedb}/` (skipping `__tests__`, `generated/`, `module_bindings/`). Compares file presence + per-file export sets. 31 file-level architectural exemptions documented.

**77 shared files · 4 cvx-only (0 unaccounted) · 22 stdb-only (0 unaccounted) · 0 files with symbol-set divergence.** Status: 🟢 zero unaccounted-for gaps.
