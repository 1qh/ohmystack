...
| `log`       | 390 / 256          | 71 / 73             | 10 / 10          | 357L     | 5 / 5 ✓         |
...
**238 shared files · 45 cvx-only · 35 stdb-only · 1 files with cross-backend symbol divergence.** Status: 🔴 3 unaccounted-for gap(s).

| Pair                     | shared    | cvx-only           | stdb-only          | symbol gaps | status |
| ------------------------ | --------: | ------------------ | ------------------ | ----------: | ------ |
| **lib/noboil/src**       | 78        | 6 (2 unaccounted)  | 22 (0 unaccounted) | 1           | 🔴     |
...
- **lib/noboil/src**: cvx-only file `server/budget.ts`, `__tests__/budget.test.ts`
- `lib/noboil/src/server/index.ts` — cvx-only: `makeBudget`, `periodKeyFor` · stdb-only: —
