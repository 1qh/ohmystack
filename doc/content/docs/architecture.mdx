...
Slots with their auto-injected fields:
...
| `log` | `log` | `parent`, `seq`, `userId`, `idempotencyKey?`, `createdAt` | `by_parent_seq`, `by_idempotency` | Append-only event log with atomic seq + idempotency ([log](./log)) |
| `kv` | `kv` | `key`, `updatedAt`, `deletedAt?` | `by_key` (unique) | Named key → value with role-gated writes ([kv](./kv)) |
| `quota` | `quota` | `owner`, `timestamps[]` | `by_owner` (unique) | Sliding-window rate limit primitive ([quota](./quota)) |
