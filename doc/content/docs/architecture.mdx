...
**238 shared files · 66 cvx-only · 35 stdb-only · 1 files with cross-backend symbol divergence.** Status: 🔴 24 unaccounted-for gap(s).

| Pair                     | shared    | cvx-only            | stdb-only          | symbol gaps | status |
| ------------------------ | --------: | ------------------- | ------------------ | ----------: | ------ |
| **lib/noboil/src**       | 78        | 27 (23 unaccounted) | 22 (0 unaccounted) | 1           | 🔴     |
...
- **lib/noboil/src**: cvx-only file `tools/_lib/caller-runtime.ts`, `tools/_lib/define-provider.ts`, `tools/_lib/prompt-blocks.ts`, `tools/_lib/http.ts`, `tools/_lib/validate.ts`, `tools/_lib/builder.ts`, `tools/_lib/parser.ts`, `tools/_lib/types.ts`, `tools/_lib/__tests__/dispatch.test.ts`, `tools/_lib/__tests__/manifest.test.ts`, `tools/_lib/__tests__/parser.test.ts`, `tools/_lib/__tests__/boundary.test.ts`, `tools/_lib/__tests__/step-sink.test.ts`, `tools/_lib/__tests__/to-dispatch-error.test.ts`, `tools/_lib/__tests__/error.test.ts`, `tools/_lib/manifest.ts`, `tools/_lib/hermetic.ts`, `tools/_lib/index.ts`, `tools/_lib/error.ts`, `server/budget.ts`, `server/audit.ts`, `__tests__/audit.test.ts`, `__tests__/budget.test.ts`
- `lib/noboil/src/server/index.ts` — cvx-only: `makeAudit`, `makeBudget`, `periodKeyFor` · stdb-only: —
