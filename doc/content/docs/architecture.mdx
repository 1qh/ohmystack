...
| Brand | Wrapper | Auto-injected fields | Indexes | Description |
|---|---|---|---|---|
| `base` | `baseTable / cacheCrud` | `updatedAt? (optional)` | `(none — keyed by upstream id)` | External-data cache (TTL, refresh, invalidate) |
| `child` | `childTable / childCrud` | `parentId`, `updatedAt` | `by_parent` | Child-of-parent CRUD with cascade options |
| `kv` | `kvTable / kv` | `key`, `updatedAt`, `createdAt`, `deletedAt? (when softDelete)` | `by_key (unique)` | Named key → value, public reads + role-gated writes |
| `log` | `logTable / log` | `parent`, `seq`, `userId`, `createdAt`, `idempotencyKey?`, `deletedAt? (when softDelete)` | `by_parent_seq`, `by_idempotency` | Append-only event log with atomic seq + idempotency |
| `org` | `orgTable / orgCrud` | `userId`, `orgId`, `updatedAt` | `by_org`, `by_org_user` | Org-scoped CRUD with membership + role checks |
| `orgDef` | `orgTables (via setup)` | `(n/a — org definition itself)` | `(n/a)` | The org definition (passed as orgSchema to noboil()) |
| `owned` | `ownedTable / crud` | `userId`, `updatedAt` | `by_user` | User-owned CRUD |
| `quota` | `quotaTable / quota` | `owner`, `timestamps[]` | `by_owner (unique)` | Sliding-window rate limit primitive |
| `singleton` | `singletonTable / singletonCrud` | `userId`, `updatedAt` | `by_user` | One row per user (get + upsert) |
