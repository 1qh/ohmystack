...
SpacetimeDB uses `@tanstack/react-form` directly with `zodFromTable` for validation. There are no prebuilt field components. Build fields with the `form.Field` render prop.
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); form.handleSubmit() }}>
      ...
      <button type="submit" disabled={form.state.isSubmitting}>
        {form.state.isSubmitting ? 'Saving...' : 'Create'}
      </button>
    </form>
  )
}
...
SpacetimeDB doesn't ship prebuilt field components. Build fields with `form.Field` from `@tanstack/react-form`. The `zodFromTable` schema drives validation. Field types come from your SpacetimeDB column definitions.
...
```tsx
import { buildMeta, getMeta } from '@noboil/spacetimedb/react'
```
...
Handle conflicts manually in `onSubmit`. The reducer returns `CONFLICT` when the row was modified since the client last read it.
...
onSubmit: async ({ value }) => {
  try {
    ...
    if (data?.code === 'CONFLICT') {
      toast.error('Someone else edited this. Reload to see the latest version.')
    }
  }
}
```
...
Implement multi-step forms with local state and separate `useForm` instances per step.
