...
| `File` | `cvFile()` | File upload | `name`, `label`, `accept`, `className` |
| `Files` | `cvFiles()` | Multi-file upload | `name`, `label`, `accept`, `maxFiles`, `className` |
...
The only file-field difference is the upload backend: `File` / `Files` upload to S3 (or MinIO in local dev) via a presigned URL route instead of Convex storage. The schema field type is still `cvFile()` / `cvFiles()` from `@noboil/spacetimedb/schema`.
