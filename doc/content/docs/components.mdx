...
Both `@noboil/convex/components` and `@noboil/spacetimedb/components` export the same component set. Snippets are written in single voice — pick the import path that matches your backend. The only file-field difference (Convex storage vs S3 presigned URLs) is documented in [Convex vs SpacetimeDB](./differences#3-file-storage).
...
```tsx
import { useForm } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
Wires a form directly to a server mutation, pre-fills values from an existing record, and surfaces server errors as field-level validation. The Convex shape takes a `mutation` reference; the SpacetimeDB shape takes a `mutate` callback (because reducers are called explicitly via `useReducer`).
...
```tsx
// SpacetimeDB
import { useFormMutation } from '@noboil/spacetimedb/components'
...
| Param | Type | Description |
|-------|------|-------------|
| `mutation` (Convex) | `MutationReference` | Convex mutation to call on submit |
| `mutate` (SpacetimeDB) | `(data) => Promise<unknown>` | Reducer wrapper to call on submit |
...
```tsx
import { Form } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
All 16 field components are available inside the `Form` render prop on both backends. Every field accepts `label`, `placeholder`, `disabled`, and `className`.
...
The `<File>` and `<Files>` components handle the upload backend automatically — Convex storage on `@noboil/convex`, S3/MinIO presigned URLs on `@noboil/spacetimedb`. Schema field type is `file()` / `files()` from the matching `/schema` import on both.
...
```tsx
import { ConflictDialog } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
```tsx
import { EditorsSection } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
```tsx
import { PermissionGuard } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
```tsx
import { OrgAvatar } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
```tsx
import { RoleBadge } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
```tsx
import { OfflineIndicator } from '@noboil/convex/components'  // or @noboil/spacetimedb/components
...
Paginated data fetching with cursor-based or offset-based pagination. See [Data fetching](./data-fetching) for the full options reference and the SpacetimeDB-specific shape.

```tsx
import { useList } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
...
```tsx
import { useInfiniteList } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
...
```tsx
import { useBulkSelection } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
...
```tsx
import { useBulkMutate } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
...
```tsx
import { useSearch } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
...
```tsx
import { useSoftDelete } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
