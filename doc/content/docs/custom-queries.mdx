...
description: Escape hatches for custom server logic — pq/q/m builders (Convex), procedures, SQL API, and scheduled reducers (SpacetimeDB).
---
...
```typescript
const STDB_URL = 'http://localhost:%%port:stdb%%'
const MODULE = 'my-app'
