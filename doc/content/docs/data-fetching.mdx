...
For per-resource sliding-window quotas (anti-spam, ballot-stuffing, API throttling), use the [`quota` factory](./quota) instead — it gives you `check`/`record`/`consume` triad and a `useQuota()` hook with reactive remaining count.

## Append-only logs

For append-only data (chat messages, vote ballots, audit trails) use the [`log` factory](./log). The `useLog()` hook provides paginated rows + `append`/`appendBulk`/`purge`/`restore` helpers. Each log row gets an atomic per-parent `seq` and optional idempotency key.
...
For state addressed by a known key (banner, feature flag, system status) use the [`kv` factory](./kv). The `useKv()` hook subscribes to one key and returns `data` + `update`/`remove`/`restore`.
