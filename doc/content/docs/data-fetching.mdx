...
Both backends are reactive: subscribed components re-render whenever underlying data changes. The user-facing API for filtering, sorting, paginating, and searching is the same on both libs — `useList` from `/react`. Where they differ is the underlying transport (Convex query subscription vs SpacetimeDB WebSocket subscription) and the visibility model (`pub`/`auth` destructure vs `clientVisibilityFilter` SQL). See [Convex vs SpacetimeDB](./differences#2-where-filters-and-the-pub-option) for the details.

## Access control on read endpoints

Both libs accept the same `pub` table option, but apply it differently. Convex generates two read endpoints (`pub.list` and `auth.list`); SpacetimeDB generates a server-enforced `clientVisibilityFilter` SQL rule.
...
`useList` is the high-level hook on both backends — it handles filtering, sorting, search, pagination. The lower-level `useQuery` (Convex) / `useTable` (SpacetimeDB) hooks are also available for direct access.
...
Both backends accept the same where-clause shape: field equality, comparison operators (`$gt`, `$gte`, `$lt`, `$lte`, `$between`), the `own: true` shorthand, and `or` groups.
...
`file()` for single file, `files()` for arrays — same name, same schema, both backends. The form components handle the upload backend automatically. See [File storage](./differences#3-file-storage) for the Convex storage vs S3/MinIO presigned URL differences.

## Soft delete and undo

Enable on any table by adding `softDelete: true` to the table options. Both backends generate a `restore` endpoint alongside `rm`, and `useSoftDelete` provides the undo-toast UX.
...
For 1:1 per-user data — profiles, settings, preferences. Each user gets exactly one record. Same API on both backends.
...
Same API on both backends. Child tables inherit access control from their parent.
...
Both backends accept `rateLimit` on table options. See [Security](./security#rate-limiting) for the full reference.
...
Both libs export `Devtools` from `/react`. In dev mode it auto-mounts; you can also render it standalone:

```tsx
import { Devtools } from '@noboil/convex/react'  // or @noboil/spacetimedb/react
