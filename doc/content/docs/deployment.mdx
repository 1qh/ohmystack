...
- SpacetimeDB at `ws://localhost:4000` (WebSocket) and `http://localhost:4000` (HTTP SQL API)
- MinIO at `http://localhost:4600` (S3 API) and `http://localhost:4601` (console)
...
```bash
NEXT_PUBLIC_SPACETIMEDB_URL=ws://localhost:4000
NEXT_PUBLIC_MODULE_NAME=my-app
SPACETIMEDB_URL=http://localhost:4000
MODULE_NAME=my-app
...
```bash
mc alias set local http://localhost:4600 minioadmin minioadmin
...
curl http://localhost:4000/database/ping
```
