...
| | Convex | SpacetimeDB |
|---|---|---|
| Storage | Convex native (`_storage` IDs) | S3 / MinIO via presigned URLs |
| Setup | `makeFileUpload()` in your `lazy.ts` + `FileApiProvider` in providers.tsx | `createS3UploadPresignedUrl()` from a Next.js API route + `useUpload()` on the client |
| Cleanup on delete | Automatic via cascade hooks | Manual or via a scheduled reducer |
| Defaults | 10MB per file, image compression to 1920px, 10 uploads/min rate limit | Configurable in your presign route |

The `<File>` and `<Files>` form components from `/components` work identically — they just hand the upload off to the right backend underneath.
