...
SpacetimeDB inline files produce blob URLs that expire on reload. Use `resolveFileUrl(files, ref)` from `/react` to reconstruct URLs from the subscribed file table — Convex storage URLs are permanent and don't need this step.
