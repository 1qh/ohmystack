...
- `useFormMutation` takes your Zod schema and wires up the reducer call
- `name='title'` is checked at compile time — `name='titl'` is a type error
- `Choose` auto-generates options from the Zod enum
- `Toggle` knows `published` is a boolean field — using `<Text name='published' />` would be a type error
- Zod validation runs on submit — `title.min(1)` enforces non-empty
...
Your schema has `coverImage: file().nullable().optional()`. The `File` field handles upload automatically:
...
Switch to `useFormMutation` which wires up the reducer and pre-fills values:
...
- `pickValues` extracts schema-matching fields from the row (ignores `id`, `userId`, `updatedAt`)
- `transform` adds the `id` and `expectedUpdatedAt` fields before submitting
- Empty optional strings auto-coerce to `undefined`
...
Pass `expectedUpdatedAt` when calling the update reducer. The `Form` component includes a built-in `ConflictDialog` that handles conflicts automatically:
...
const form = useFormMutation({
  schema: postSchema,
  values: pickValues(postSchema, post),
  mutate: d => updatePost({ id: post.id, ...d }),
  autoSave: { enabled: true, debounceMs: 1000 }
})
```
...
```tsx
import { AutoSaveIndicator } from '@noboil/spacetimedb/components'
;<AutoSaveIndicator lastSaved={form.lastSaved} />
```

This shows "Saved" that updates in real-time.
...
| `File`        | `file()`                     | File picker with upload          |
| `Files`       | `files()`                    | Multi-file picker                |
| `Arr`         | `array(string())`              | Tag input                        |
| `Datepick`    | `date()`                       | Date picker                      |
| `Combobox`    | `string()` with options        | Searchable dropdown              |
| `Colorpick`   | `string()`                     | Color picker                     |
| `MultiSelect` | `array(string())`              | Multi-select dropdown            |
| `Rating`      | `number()`                     | Star rating                      |
| `Slider`      | `number()`                     | Range slider                     |
| `Timepick`    | `string()`                     | Time picker                      |
| `Submit`      | --                             | Submit button with loading state |
| `Err`         | --                             | Field error display              |
...
SpacetimeDB uses the same field components as Convex, available via `<Form render={...} />`:
...
| `Choose`      | `enum()`                       | Select dropdown                  |
| `Toggle`      | `boolean()`                    | Checkbox or switch               |
| `File`        | `file()`                     | File picker with S3 upload       |
| `Files`       | `files()`                    | Multi-file picker                |
| `Arr`         | `array(string())`              | Tag input                        |
| `Datepick`    | `date()`                       | Date picker                      |
| `Combobox`    | `string()` with options        | Searchable dropdown              |
...
**Reducer errors** -- `useFormMutation` shows error toasts by default:
...
**Toast shorthand** -- pass a string to `toast` for automatic success/error handling:
