...
It ships as two libraries with **the same public API** and almost identical call sites:

- **`@noboil/convex`** — Convex backend (document store, automatic caching, optimistic updates)
- **`@noboil/spacetimedb`** — SpacetimeDB backend (relational, WebSocket subscriptions, in-memory)

The two libraries export the same `noboil()`, `schema()`, `useForm`, `Form`, `useList`, `defineSteps`, `EditorsSection`, and so on. The handful of places where they genuinely differ live in [Convex vs SpacetimeDB](./differences). The rest of the docs are written in single voice — what works on one library works on the other.
...
```ts
import { schema, file, files } from '@noboil/convex/schema'  // or @noboil/spacetimedb/schema
...
```ts
import { noboil } from '@noboil/convex/server'  // or @noboil/spacetimedb/server
// ...
