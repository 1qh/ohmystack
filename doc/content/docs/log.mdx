...
```ts
import { object, string, number } from 'zod/v4'
...
- `parent: Id<parentTable>` — foreign key (indexed)
- `seq: number` — monotonic per parent, starts at 1
- `idempotencyKey?: string` — optional dedup hash
- `userId: Id<users>` — author
- `createdAt: number` / `updatedAt: number` — timestamps
- `deletedAt?: number` — set when `softDelete: true` and row is purged
- `_creationTime: number` — Convex native

Indexes auto-created: `by_parent_seq`, `by_idempotency` (if seen).
...
const api = noboil({
  ...
  tables: ({ table }) => ({
    ...
    vote: table(s.vote, {
      ...
      hooks: {
        beforeAppend: async (ctx, { parent, payload }) => {
          // mutate payload before insert
          return { ...payload, voter: ctx.user._id }
        },
        afterAppend: async (ctx, { row }) => {
          // notify subscribers, increment counters, ...
        },
...
export const { append, appendBulk, auth, authIndexed, list, listAfter, purgeByParent,
                read, restoreByParent, rm, update } = api.vote
```
...
Counter row per parent stored in internal `_logCounters` table (one row per `{table, parent}`). `append` reads counter, increments atomically, inserts row. Convex mutations are transactional so the read+write is safe. SpacetimeDB uses `auto_inc`.
...
const s = schema({
  ...
  log: {
    message: { parent: 'chat', schema: object({ content: string(), role: z.enum(['user','assistant','system']) }) }
  }
})
```
...
The [poll demo](https://github.com/1qh/noboil/tree/main/web/cvx/poll) uses `log` for vote rows: each ballot is appended with `{optionIdx, voter}`, listed paginated, purged + restored as admin actions, and counted via `useLog().data` aggregation.
