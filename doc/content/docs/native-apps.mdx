...
All mobile apps reuse the **same backend packages** as web (`backend/convex`, `backend/spacetimedb`) with no mobile-specific backend code.

### Mobile Architecture

```
lib/rnr/         32 React Native Reusables components (synced, READ-ONLY)
lib/femobile/   Mobile providers, layouts, and shared components
expo/cvx/   4 Expo apps (Convex backend)
expo/stdb/  4 Expo apps (SpacetimeDB backend)
```
...
- **React Native Reusables** (RNR) components from `lib/rnr`
...
Verify parity: `bash expo/maestro/test-count-check.sh`
...
```bash
bunx noboil-convex codegen-swift --schema backend/convex/t.ts --convex backend/convex/convex \
  --output swiftcore/Sources/ConvexCore/Generated.swift \
  --mobile-output mobile/convex/convex-shared/Sources/ConvexShared/MobileAPI.swift
```
...
The codegen output depends on a thin protocol in `swiftcore`:
