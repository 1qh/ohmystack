...
```tsx
import { makeOrgScoped } from '@noboil/convex/server'
...
Build your own org UI components using the hooks from `@noboil/spacetimedb/react`. The `useOrg`, `useActiveOrg`, and `useMyOrgs` hooks provide all the data you need.
