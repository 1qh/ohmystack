...
The template ships `spacetimedb.yml` (Docker Compose) and a `backend/spacetimedb` workspace with `t.ts` (Zod schemas wrapped with `schema()`) and `src/index.ts` (the `noboilStdb()` module definition). Boot SpacetimeDB + MinIO and publish the module:
