...
Convex uses its built-in file storage via `file()` in the schema — no presigned URL flow is needed. SpacetimeDB stores files in S3 / MinIO via a presigned upload route. Both backends accept `file()` in the Zod schema, but the wiring on the SpacetimeDB side is shown here.
...
Both libraries support singleton tables. The Convex version uses `singletonCrud()` and `file()` (Convex storage); the SpacetimeDB version uses the same `singleton: { profile }` schema and `file()` (S3 storage). The Convex flow is shown below.
