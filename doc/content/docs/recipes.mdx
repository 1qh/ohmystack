...
Convex uses native storage via `file()` — no presigned URL flow needed. SpacetimeDB stores files in S3 / MinIO via a presigned upload route. The schema field is `file()` either way; this section shows the SpacetimeDB wiring.
...
`singletonCrud` from a `singleton: { profile }` schema slot. The Convex flow is shown below; the SpacetimeDB version uses the same schema with S3 storage instead of native.
