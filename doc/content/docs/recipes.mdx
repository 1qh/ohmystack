...
## Unified CRUD with useCrud

`useCrud` gives you data + mutations in one hook call — same API on both backends:
...
const BlogList = () => {
  const { data, create, update, rm, hasMore, loadMore, isLoading } = useCrud({
...
```tsx
// SpacetimeDB — same shape, different refs
import { useCrud } from '@noboil/spacetimedb/react'
