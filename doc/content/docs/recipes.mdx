...
SpacetimeDB doesn't have a direct equivalent for this recipe. For blog-style owned CRUD with file upload, see the [File Upload](#file-upload-with-s3-pre-signed-urls) and [Org-scoped Data](#org-scoped-data-access) recipes, then combine them with `useList` filtering.
...
> Available for SpacetimeDB only. Convex users can build a similar pattern with `crud` + `useList` + presence.
...
> Available for SpacetimeDB only. Convex users get native file storage via `cvFile()` in the schema.
...
> Available for Convex only. SpacetimeDB users can compose `useMut` calls across steps manually.
...
> Available for Convex only. SpacetimeDB users can use `orgScoped` with a single-member org or a `base` table with a unique user constraint.
...
## Custom Queries Alongside CRUD

> Available for Convex only. SpacetimeDB users write custom reducers and queries directly in Rust.
...
> Available for Convex only. SpacetimeDB reducers run server-side, so bulk operations are handled in a single reducer call.
...
> Available for Convex only. SpacetimeDB users can filter rows client-side using `useList` with a `where` clause on `userId`.
