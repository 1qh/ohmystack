...
while IFS= read -r dir; do
  [ -z "$dir" ] && continue
  rel="${dir#$ROOT_DIR/expo/}"
  db="${rel%%/*}"
  app="${rel##*/}"
  all_targets+=("$db/$app")
done < <(for d in "$ROOT_DIR"/expo/*/*; do [ -d "$d/e2e" ] && printf '%s\n' "$d"; done)

get_smoke_flow() {
  e2e_dir="$ROOT_DIR/expo/$1/$2/e2e"
  best_flow=''
  fallback_flow=''
...
if [ "${#targets[@]}" -gt 0 ]; then
  ...
  if [ "${#changed_files[@]}" -eq 0 ]; then
    ...
    for p in "${changed_files[@]}"; do
      case "$p" in
        expo/cvx/blog/*) add_target cvx blog ;;
        expo/cvx/chat/*) add_target cvx chat ;;
        expo/cvx/movie/*) add_target cvx movie ;;
        expo/cvx/org/*) add_target cvx org ;;
        expo/stdb/blog/*) add_target stdb blog ;;
        expo/stdb/chat/*) add_target stdb chat ;;
        expo/stdb/movie/*) add_target stdb movie ;;
        expo/stdb/org/*) add_target stdb org ;;
        expo/maestro/*) for t in "${all_targets[@]}"; do add_target "${t%%/*}" "${t##*/}"; done ;;
        lib/femobile/*|backend/convex/*|backend/spacetimedb/*|lib/convex/*|lib/spacetimedb/*|lib/shared/*)
          for t in "${all_targets[@]}"; do add_target "${t%%/*}" "${t##*/}"; done
          ;;
        expo/cvx/*) add_db_group cvx ;;
        expo/stdb/*) add_db_group stdb ;;
      esac
    done
  fi
fi
...
get_bundle_id() {
  app_json="$ROOT_DIR/expo/$1/$2/app.json"
  python3 - <<PY
import json
from pathlib import Path
p = Path('$app_json')
...
while IFS= read -r key; do
  ...
  flow_path="$ROOT_DIR/expo/$db/$app/e2e/$flow_file"
