...
const makePost =
  ...
  async (req: NextRequest): Promise<NextResponse> => {
    try {
      ...
      const { options, storageId, thumbnail } = body
...
const getCachedClient = (url: string): ConvexHttpClient => {
...
const makeImageRoute = async ({ convexUrl, fileInfoQuery = 'file:info' }: ImageRouteConfig) => {
  await Promise.resolve()
  const getClient = () => getCachedClient(convexUrl)
  const opts = { getClient, queryRef: fileInfoQuery }
