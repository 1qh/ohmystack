...
const DEFAULT_MAX_FILE_SIZE = 10 * BYTES_PER_MB
const CHUNK_SIZE = 5 * BYTES_PER_MB
 ...
 */
const makeFileUpload = <DM extends GenericDataModel>(config: FileUploadConfig<DM>) => {
  const {
    action,
  ...
  const tPath = (anyApi as Rec)[namespace] as Rec
  const authUserId = async (ctx: unknown) => getAuthUserId(ctx)
  ...
  const validateFileSize = async (storage: { delete: (id: string) => Promise<void> }, id: string, size: number) => {
    if (size > maxFileSize) {
      await storage.delete(id)
      throw cvErr('FILE_TOO_LARGE', `File size ${size} exceeds ${maxFileSize} bytes`)
    }
  }
  const checkRateLimit = async (db: DbLike, userId: string) => {
    const now = Date.now()
  ...
  const startChunkedUpload = mutation({
    args: {
  ...
  const getSessionForAssembly = internalQuery({
  ...
  const finalizeAssembly = internalMutation({
    args: {
    ...
    handler: async (
      ...
      const chunks = await c.db
        ...
        .collect()
      const sr = await Promise.allSettled(chunkStorageIds.map(async (id: string) => c.storage.delete(id)))
      for (const r of sr)
  ...
  const assembleChunks = action({
    ...
    handler: async (
      ...
      const combinedBlob = new Blob(chunkBlobs, { type: session.contentType as string })
      const finalStorageId = await c.storage.store(combinedBlob)
  ...
  const cancelChunkedUpload = mutation({
    args: { uploadId: v.string() },
    handler: async (c: FileCtx, { uploadId }: { uploadId: string }) => {
      ...
      const chunks = await c.db
        ...
        .collect()
      const sr = await Promise.allSettled(chunks.map(async (chunk: Rec) => c.storage.delete(chunk.storageId as string)))
      for (const r of sr)
        if (r.status === 'rejected') log('warn', 'file:chunk_cleanup_failed', { reason: String(r.reason) })
      await Promise.all(chunks.map(async (chunk: Rec) => c.db.delete(chunk._id as string)))
