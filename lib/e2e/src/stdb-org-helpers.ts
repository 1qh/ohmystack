...
const SNAKE_TO_CAMEL_RE = /_(?<ch>[a-z])/gu,
  ...
  REDUCER_NAME_RE = /REDUCER_CALL_FAILED\((?<reducer>[^)]+)\)/u,
  API_PATH_RE = /api\.(?<mod>\w+)\.(?<fn>\w+)/u
...
const userTokens = new Map<string, string>(),
  ...
  getSqlFields = (schema: SqlSchema | undefined): string[] => {
    if (!schema) return []
  ...
  rowToObject = (row: unknown, fields: string[]): Record<string, unknown> => {
  ...
  httpSql = async (sql: string, token: string): Promise<Record<string, unknown>[]> => {
    const ctx = getHttpCtx(),
  ...
  queryRows = async (apiPath: string, args: Record<string, unknown>): Promise<unknown> => {
    ...
    const rawRows = await httpQuery(tableName, token),
      rows = rawRows.map(r => normalizeRow(r))
    if (apiPath === 'org.get') {
    ...
    if (apiPath === 'org.membership') {
      const orgMembers = rows.filter(r => String(r.orgId) === String(args.orgId)),
        orgRows = await httpQuery('org', getHttpCtx().token),
        normalizedOrgs = orgRows.map(r => normalizeRow(r)),
    ...
    if (apiPath === 'orgProfile.get') {
      try {
        const fs = await import('node:fs'),
          tokenFileContent = fs.readFileSync(`${process.cwd()}/e2e/.stdb-test-token.json`, 'utf8'),
          parsed = JSON.parse(tokenFileContent) as IdentityResponse,
          { identity } = parsed,
    ...
    if (apiPath === 'project.list') {
      const filtered = args.orgId ? rows.filter(r => String(r.orgId) === String(args.orgId)) : rows,
        result: ListResult = { isDone: true, page: filtered }
      return result
    }
  ...
  mutationReducerMap: Record<string, string> = {
  ...
  handleMutationResult = async (apiPath: string, args: Record<string, unknown>, token: string): Promise<unknown> => {
    if (apiPath === 'org.create') {
      const data = (args.data ?? args) as Record<string, unknown>,
        orgs = await httpQuery('org', token),
        org = orgs.find(o => o.slug === data.slug),
        result: OrgCreateResult = { orgId: org ? String(org.id) : '' }
      return result
    }
    ...
    if (apiPath === 'project.create') {
      const projects = await httpQuery('project', token),
        filtered = projects.filter(p => Number(p.org_id) === toU32(args.orgId))
      let maxId = 0
    ...
    if (apiPath === 'task.create') {
      const tasks = await httpQuery('task', token),
        filtered = tasks.filter(t => Number(t.org_id) === toU32(args.orgId))
      let maxId = 0
    ...
    if (apiPath === 'wiki.create') {
      const wikis = await httpQuery('wiki', token),
        filtered = wikis.filter(w => Number(w.org_id) === toU32(args.orgId))
      let maxId = 0
