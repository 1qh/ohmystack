...
const userTokens = new Map<string, string>(),
  ...
  httpQuery = async (tableName: string, token: string): Promise<unknown[]> => httpSql(`SELECT * FROM ${tableName}`, token),
  getSqlFields = (schema: unknown): string[] => {
    if (!schema || typeof schema !== 'object') return []
    const s = schema as Record<string, unknown>,
      elements =
        s.elements ??
        (s.Product && typeof s.Product === 'object' ? (s.Product as Record<string, unknown>).elements : undefined),
      fields: string[] = []
  ...
  rowToObject = (row: unknown, fields: string[]): unknown => {
    ...
    for (let i = 0; i < fields.length; i += 1) if (fields[i]) result[fields[i]!] = row[i]
    return result
  },
  snakeToCamel = (s: string): string => s.replaceAll(/_([a-z])/gu, (_, c: string) => c.toUpperCase()),
  ...
  unwrapOption = (v: unknown): unknown => {
    ...
    if (v && typeof v === 'object' && 'some' in (v as Record<string, unknown>)) return (v as { some: unknown }).some
    if (v && typeof v === 'object' && 'none' in (v as Record<string, unknown>)) return
    return v
  },
  stripHexPrefix = (v: unknown): unknown => {
  ...
  toDoubleOption = (v: unknown): { none: [] } | { some: { none: [] } | { some: unknown } } =>
  ...
  extractErrorCode = (e: unknown): null | { code: string } => {
    if (e instanceof Error) {
      const match =
        /REDUCER_CALL_FAILED\([^)]*\):\s*(?!The\s)(?<code>[A-Z_]+)/u.exec(e.message) ??
        /(?:code[":]+\s*)(?<code>[A-Z_]+)/u.exec(e.message)
      if (match?.groups?.code) return { code: match.groups.code }
      if (/fatal error/iu.test(e.message)) {
        const reducerMatch = /REDUCER_CALL_FAILED\((?<reducer>[^)]+)\)/u.exec(e.message),
          inferredCodes: Record<string, string> = {
          ...
          reducer = reducerMatch?.groups?.reducer ?? ''
  ...
  makeTc = () => {
    const mutationReducerMap: Record<string, string> = {
      ...
      buildMutationArgs = (apiPath: string, args: Record<string, unknown>): unknown[] => {
        ...
        switch (apiPath) {
          ...
          case 'task.rm':
            return [toU32(args.id)]
          case 'task.toggle':
            return [toU32(args.id), { some: { some: true } }, { none: [] }, { none: [] }, { none: [] }]
          case 'wiki.create':
            return [
              toU32(args.orgId),
      ...
      queryRows = async (apiPath: string, args: Record<string, unknown>): Promise<unknown> => {
        const { token } = getHttpCtx(),
        ...
        if (apiPath === 'org.membership') {
          const orgMembers = rows.filter(r => String(r.orgId) === String(args.orgId)),
            orgRows = await httpQuery('org', getHttpCtx().token),
            normalizedOrgs = orgRows.map(r => normalizeRow(r as Record<string, unknown>)),
            org = normalizedOrgs.find(o => String(o.id) === String(args.orgId))
          if (org)
            return {
              role: 'owner',
        ...
        if (apiPath === 'orgProfile.get') {
          const tokenFile = await import('node:fs')
            .then(fs => fs.readFileSync(`${process.cwd()}/e2e/.stdb-test-token.json`, 'utf8'))
            .catch(() => null)
          if (tokenFile) {
            const { identity } = JSON.parse(tokenFile) as {
                identity: string
              },
              match = rows.find(r => String(r.userId).includes(identity) || String(r.user_id).includes(identity))
    ...
    return {
      mutation: async <T>(apiRef: unknown, args: Record<string, unknown>): Promise<T> => {
        ...
        if (apiPath === 'org.invite') {
          const invites = (await httpQuery('org_invite', token)) as Record<string, unknown>[],
            filtered = invites.filter(i => Number(i.org_id) === toU32(args.orgId)),
        ...
        if (apiPath === 'project.create') {
          const projects = (await httpQuery('project', token)) as {
            ...
            filtered = projects.filter(p => p.org_id === toU32(args.orgId))
          let maxId = 0
          for (const p of filtered) if (p.id > maxId) maxId = p.id
          return maxId > 0 ? (String(maxId) as T) : ('' as T)
        }
        if (apiPath === 'project.update') {
          const projects = (await httpQuery('project', token)) as Record<string, unknown>[],
        ...
        if (apiPath === 'task.create') {
          const tasks = (await httpQuery('task', token)) as {
            ...
            filtered = tasks.filter(t => t.org_id === toU32(args.orgId))
          let maxId = 0
          for (const t of filtered) if (t.id > maxId) maxId = t.id
          return maxId > 0 ? (String(maxId) as T) : ('' as T)
        }
        if (apiPath === 'task.toggle') {
          const tasks = (await httpQuery('task', token)) as Record<string, unknown>[],
        ...
        if (apiPath === 'wiki.create') {
          const wikis = (await httpQuery('wiki', token)) as {
            ...
            filtered = wikis.filter(w => w.org_id === toU32(args.orgId))
          let maxId = 0
