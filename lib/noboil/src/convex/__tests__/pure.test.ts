...
describe('doctor', () => {
  ...
  test('checkDeps — all present', () => {
    expect(
      checkDeps({
        dependencies: { convex: '1', 'noboil/convex': '2', zod: '3' }
      }).status
    ).toBe('pass')
  })
  ...
  test('checkDeps — devDependencies count', () => {
    expect(
      checkDeps({
        devDependencies: { convex: '1', 'noboil/convex': '2', zod: '3' }
      }).status
    ).toBe('pass')
  })
...
describe('makeLog factory schema', () => {
  ...
  test('payload schema validates required fields', () => {
    const okLog = logs.vote.schema.safeParse({ optionIdx: 0, voter: 'x' })
    expect(okLog.success).toBe(true)
    const badLog = logs.vote.schema.safeParse({ optionIdx: 'string-not-number', voter: 'x' })
