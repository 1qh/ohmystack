...
const makeInviteHandlers = ({ m, q }: { m: Mb; q: Qb }) => {
  const invite = m({
    ...
    handler: async (c: Rec, { email, isAdmin, orgId }: { email: string; isAdmin: boolean; orgId: string }) => {
      ...
      const inviteId = await (c.db as DbLike).insert('orgInvite', {
        email,
        expiresAt,
        isAdmin,
        orgId,
  ...
  const acceptInvite = m({
    ...
    handler: async (c: Rec, { token }: { token: string }) => {
      ...
      const orgDoc = await db.get(inviteDoc.orgId as string)
