...
const makeJoinHandlers = ({ m, q }: { m: Mb; q: Qb }) => {
  ...
  const approveJoinRequest = m({
    ...
    handler: async (c: Rec, { isAdmin, requestId }: { isAdmin?: boolean; requestId: string }) => {
      const db = c.db as DbLike
      const requestDoc = await db.get(requestId)
  ...
  const pendingJoinRequests = q({
    ...
    handler: async (c: Rec, { orgId }: { orgId: string }): Promise<JoinRequestItem[]> => {
      ...
      const requests = await db
        ...
        .collect()
      const users = await Promise.all(requests.map(async (r: Rec) => db.get(r.userId as string)))
      const result: JoinRequestItem[] = []
