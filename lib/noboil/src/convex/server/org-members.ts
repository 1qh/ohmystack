...
const makeMemberHandlers = ({ m, q }: { m: Mb; q: Qb }) => {
  const membership = q({
    ...
    handler: async (
      ...
      const role = getOrgRole({ member, org: orgDoc, userId })
      if (!role) return null
  ...
  const members = q({
    ...
    handler: async (c: Rec, { orgId }: { orgId: string }): Promise<OrgMemberItem[]> => {
      ...
      const ownerUser = await db.get(orgDoc.userId as string)
      result.push({
        role: 'owner',
      ...
      const memberDocs = await db
        ...
        .collect()
      const userDocs = await Promise.all(memberDocs.map(async (x: Rec) => db.get(x.userId as string)))
  ...
  const setAdmin = m({
    ...
    handler: async (c: Rec, { isAdmin, memberId }: { isAdmin: boolean; memberId: string }) => {
      const db = c.db as DbLike
      const memberDoc = await db.get(memberId)
