...
const SECOND_TEST_EMAIL = nodeEnv.NOBOIL_TEST_EMAIL_SECONDARY ?? 'other@example.test'
const EDITOR_TEST_EMAIL = nodeEnv.NOBOIL_TEST_EMAIL_EDITOR ?? 'editor@example.test'
const BATCH_SIZE = 50
...
const makeTestAuth = <DM extends GenericDataModel>(config: TestAuthConfig<DM>) => {
  ...
  const getAuthUserIdOrTest = async (ctx: unknown): Promise<null | string> => {
  ...
  const getTestUserByEmail = query({
    args: { email: v.string() },
  ...
  const addTestOrgMember = mutation({
  ...
  const removeTestOrgMember = mutation({
  ...
  const cleanupTestUsers = mutation({
  ...
  const acceptInviteAsUser = mutation({
  ...
  const removeMemberAsUser = mutation({
  ...
  const transferOwnershipAsUser = mutation({
  ...
  const requestJoinAsUser = mutation({
  ...
  const approveJoinRequestAsUser = mutation({
  ...
  const rejectJoinRequestAsUser = mutation({
  ...
  const cancelJoinRequestAsUser = mutation({
    args: { requestId: v.id('orgJoinRequest'), userId: v.id('users') },
    handler: async (ctx: { db: DbLike }, { requestId, userId }: { requestId: string; userId: string }) => {
      if (!isTestMode()) return null
  ...
  const pendingInvitesAsUser = query({
    args: { orgId: v.id('org'), userId: v.id('users') },
    handler: async (ctx: { db: DbLike }, { orgId, userId }: { orgId: string; userId: string }) => {
      if (!isTestMode()) return null
      const membership = await getOrgMembership(ctx.db, orgId, userId)
  ...
  const pendingJoinRequestsAsUser = query({
    args: { orgId: v.id('org'), userId: v.id('users') },
    handler: async (ctx: { db: DbLike }, { orgId, userId }: { orgId: string; userId: string }) => {
      if (!isTestMode()) return null
      const membership = await getOrgMembership(ctx.db, orgId, userId)
  ...
  const createExpiredInvite = mutation({
...
const checkChildAclPermission = async (
  ...
  const editors = parent ? ((parent.editors ?? []) as string[]) : []
  return editors.some(eid => eid === userId)
}
const addEditorToDoc = async (db: DbLike, itemId: string, editorId: string, orgId: string) => {
  const doc = await db.get(itemId)
  if (doc?.orgId !== orgId) return { code: 'NOT_FOUND' }
  const editors = (doc.editors ?? []) as string[]
  const alreadyEditor = editors.some((id: string) => id === editorId)
  if (alreadyEditor) return doc
  await db.patch(itemId, { editors: [...editors, editorId], ...time() })
  return db.get(itemId)
}
const removeEditorFromDoc = async (db: DbLike, itemId: string, editorId: string, orgId: string) => {
  const doc = await db.get(itemId)
  if (doc?.orgId !== orgId) return { code: 'NOT_FOUND' }
  const editors = (doc.editors ?? []) as string[]
...
const makeOrgTestCrud = <DM extends GenericDataModel>(config: OrgTestCrudConfig<DM>) => {
  ...
  const result: Rec = { createAsUser, listAsUser, rmAsUser, updateAsUser }
  if (hasAcl) {
