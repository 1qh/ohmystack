...
const createDevtoolsCore = ({ extractErrorData, getErrorDetail, getErrorMessage }: CreateDevtoolsCoreOptions) => {
  const errorStore: DevError[] = []
  const mutationStore: DevMutation[] = []
  const cacheStore = new Map<string, DevCacheEntry>()
  const subStore = new Map<number, DevSubscription>()
  ...
  const trackSubscription = (query: string, args?: Record<string, unknown>): number => {
    const id = nextId
    nextId += 1
    subStore.set(id, {
  ...
  const updateSubscription = (id: number, status: 'error' | 'loaded' | 'loading') => {
  ...
  const untrackSubscription = (id: number) => {
    subStore.delete(id)
    notify()
  }
  const updateSubscriptionData = (id: number, data: unknown[], preview: string) => {
    const sub = subStore.get(id)
    if (!sub) return
  ...
  const trackMutation = (name: string, args?: Record<string, unknown>): number => {
    const id = nextId
    nextId += 1
  ...
  const completeMutation = (id: number, status: 'error' | 'success') => {
  ...
  const getOrCreateCacheEntry = (table: string, key: string) => {
  ...
  const trackCacheAccess = (opts: { hit: boolean; key: string; stale?: boolean; table: string }) => {
