...
const clearActiveOrgCookieClient = () => {
  if (typeof document === 'undefined') return
  document.cookie = `${ACTIVE_ORG_COOKIE}=; path=/; max-age=0`
  document.cookie = `${ACTIVE_ORG_SLUG_COOKIE}=; path=/; max-age=0`
}
const useResolveActiveOrg = <O extends OrgDoc>(orgs: OrgMembership<O>[], currentOrg: O): ActiveOrgState<O> => {
  ...
  const setActiveOrg = useCallback((org: O) => {
 ...
 */
const OrgProvider = <O extends OrgDoc, M>({
  children,
  membership,
  org,
 ...
 */
const useActiveOrg = <O extends OrgDoc = OrgDoc>() => {
 ...
 */
const useMyOrgs = <O extends OrgDoc = OrgDoc>() => {
  const ctx = use(OrgContext)
 ...
 */
const useOrgMutation = <A extends Record<string, unknown>>(mutation: (args: A) => Promise<unknown>) => {
  const { orgId } = useOrg()
  return useCallback(
 ...
 */
const createOrgHooks = <O extends OrgDoc = OrgDoc, M = unknown>(config?: {
  ...
  useOrgMutation: <A extends Record<string, unknown>>(mutation: (args: A) => Promise<unknown>) => {
    ...
    const resolved = config?.orgIdForMutation ? config.orgIdForMutation(orgId) : orgId
    return useCallback(
