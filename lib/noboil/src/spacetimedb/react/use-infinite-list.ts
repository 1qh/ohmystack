 ...
 */
const useInfiniteList = <T extends Rec>(data: T[], isReady: boolean, options?: 'skip' | InfiniteListOptions<T>) => {
  ...
  const sliced = useMemo(() => sorted.slice(0, visibleCount), [sorted, visibleCount])
