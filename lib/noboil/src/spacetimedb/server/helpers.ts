...
import type { ErrorData as SharedErrorData } from '../../shared/server/helpers'
import type {
  DbLike,
  ErrorCode,
  FID,
...
const identityFromHex = (hex: string): Identity => Identity.fromString(hex)
const idToWire = String as unknown as (id: number) => string
const idFromWire = (str: string): number => {
...
const getUser = async ({
  ...
  const uid = sender ? identityToHex(sender) : await getAuthUserId(typed(ctx))
...
const rlState = new Map<string, { count: number; windowMs: number; windowStart: number }>()
const resetRateLimitState = () => {
  rlState.clear()
}
// eslint-disable-next-line @typescript-eslint/max-params
const enforceRateLimit = (tableName: string, sender: Identity, config: RateLimitConfig, timestamp?: number) => {
...
const dbInsert = async (db: DbLike, table: string, data: Record<string, unknown>) => {
  ...
  if (tableApi && typeof tableApi.insert === 'function') {
    ...
    const fallbackId = typeof data.id === 'number' || typeof data.id === 'string' ? String(data.id) : ''
    return typeof inserted === 'number' || typeof inserted === 'string' ? String(inserted) : fallbackId
  }
  return db.insert(table, data)
}
const dbPatch = async (db: DbLike, id: string, data: Record<string, unknown>) => {
  const d = db as DbLike & Record<string, unknown>
...
const dbDelete = async (db: DbLike, id: string) => {
  await db.delete(id)
}
...
const getFieldErrors = <S extends ZodObject>(e: unknown): TypedFieldErrors<S> | undefined => {
  const d = extractErrorData(e)
