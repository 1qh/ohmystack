...
>(
  ...
  const setAdminReducer = spacetimedb.reducer(
    ...
    (ctx, args) => {
      ...
      const orgMemberPk = config.orgMemberPk(orgMemberTable)
      const member = orgMemberPk.find(typedArgs.memberId)
  ...
  const removeMemberReducer = spacetimedb.reducer(
  ...
  const leaveReducer = spacetimedb.reducer({ name: 'org_leave' }, { orgId: config.builders.orgId }, (ctx, args) => {
  ...
  const transferOwnershipReducer = spacetimedb.reducer(
