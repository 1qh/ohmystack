...
const parseSchemaResponse = async (response: Response): Promise<SchemaResponse> => {
...
const pickNames = (rows?: { name?: string }[]): string[] => {
  const names: string[] = []
...
const discoverModules = async (options?: DiscoverModulesOptions): Promise<DiscoverModulesResult> => {
  ...
  const response = await fetch(`${httpUrl}/v1/database/${resolvedModuleName}/schema?version=9`)
  const parsed = await parseSchemaResponse(response)
  return {
