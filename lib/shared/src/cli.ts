...
const ansi = (code: number, s: string) => `\u001B[${String(code)}m${s}\u001B[0m`,
  ...
  parseEnumFieldDef = <T extends string>(raw: string, validTypes: Set<T>): null | ParseEnumFieldResult<T> => {
  ...
  hasFlag = (args: string[], ...flags: string[]) => {
    for (const arg of args) if (flags.includes(arg)) return true
    return false
  },
  readEqFlag = (args: string[], name: string, fallback: string): string => {
    const prefix = `--${name}=`
    for (const arg of args) if (arg.startsWith(prefix)) return arg.slice(prefix.length)
    return fallback
  },
  readArgOrEqFlag = (args: string[], name: string, fallback: string): string => {
