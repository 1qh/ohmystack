...
const createFieldsModule = ({ defaultAsyncDebounceMs = 300, defaultRatingMax = 5, dynamicFileField, errors, unwrapZod }: CreateFieldsModuleOptions) => {
const DEFAULT_ASYNC_DEBOUNCE_MS = defaultAsyncDebounceMs,
  DEFAULT_RATING_MAX = defaultRatingMax,
  ...
  DynamicFileField = dynamicFileField,
  ...
  useField = (name: string, kind: FieldKind) => {
    ...
    if (!info)
      throw new Error(
        errors?.unknownField?.({ available: Object.keys(ctx.meta), name }) ??
  ...
  return { deriveLabel, fields, FormContext, ServerFieldError }
}
