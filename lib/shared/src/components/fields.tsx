...
type Api<T extends Record<string, unknown>> = ReactFormExtendedApi<
  ...
  unknown
>
interface CreateFieldsModuleOptions {
  defaultAsyncDebounceMs?: number
  defaultRatingMax?: number
  dynamicFileField: FileFieldComponent
  errors?: {
    chooseNoEnum?: (name: string) => string
    fieldOutsideForm?: string
    unknownField?: (args: { available: string[]; name: string }) => string
    wrongKind?: (args: { actual: string; expected: string; name: string }) => string
  }
...
const createFieldsModule = ({
  ...
  errors,
  unwrapZod
}: CreateFieldsModuleOptions) => {
  const DEFAULT_ASYNC_DEBOUNCE_MS = defaultAsyncDebounceMs
  const DEFAULT_RATING_MAX = defaultRatingMax
  ...
  const DynamicFileField = dynamicFileField
  ...
  const useField = (name: string, kind: FieldKind) => {
    ...
    if (!info)
      throw new Error(
        errors?.unknownField?.({
          available: Object.keys(ctx.meta),
          name
        }) ?? `Unknown field: ${name}. Available fields: ${Object.keys(ctx.meta).join(', ') || '(none)'}`
      )
    if (info.kind !== kind)
      throw new Error(
        errors?.wrongKind?.({ actual: info.kind, expected: kind, name }) ??
          `Field ${name} has kind ${info.kind}, expected ${kind}`
      )
    return {
      form: ctx.form,
      info,
      schema: ctx.schema,
      serverErrors: ctx.serverErrors
    }
  }
  ...
  const defaultEnumOptions = (schema: ZodObject, name: string): { label: string; value: string }[] => {
    ...
    throw new Error(errors?.chooseNoEnum?.(name) ?? `Choose: field "${name}" has no enum options. Pass options prop.`)
  }
  /**
   ...
   */
  const ServerFieldError = ({ className, name, ...props }: ComponentProps<'div'> & { name: string }) => {
    const ctx = useFCtx()
    const msg = ctx.serverErrors[name]
    if (!msg) return null
    return (
      <div
  ...
  const fields = {
    ...
    Choose: ({
      ...
      const options = explicitOptions ?? defaultEnumOptions(schema, name)
      return (
  ...
  return { FormContext, ServerFieldError, deriveLabel, fields }
}
export { createFieldsModule }
