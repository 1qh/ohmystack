...
const collectSettled = <R>(settled: PromiseSettledResult<R>[]): { errors: unknown[]; results: R[] } => {
  ...
  useBulkMutate = <A, R = void>({
    ...
    const [isPending, setIsPending] = useState(false),
      ...
      toastId = useId(),
      errorHandler = resolveBulkError(options, defaultOnError),
      ...
      run = useCallback(
            ...
            if (errors.length > 0 && errorHandler) {
              ...
              if (errors.length > 1) {
                ...
                for (let i = 1; i < errors.length; i += 1)
                  console.error(`[${packageName}] Bulk error ${i + 1}:`, errors[i]) // eslint-disable-line no-console
              }
            }
            if (results.length > 0) {
              if (toastCfg?.success) {
        ...
        [bulkMax, defaultOnError, errorHandler, mutate, options, packageName, toastCfg, toastId]
      )
