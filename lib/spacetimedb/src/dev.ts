...
const extractFlagValue = (script: string, flag: string): null | string => {
...
const detectModuleDirFromScripts = (pkg: Record<string, unknown>): string => {
...
const parseDevFlags = (args: string[]): DevFlags => {
...
const detectDevServerCommand = (pkg: null | Record<string, unknown>): { args: string[]; command: string } => {
  if (pkg) {
    const scriptsObj = pkg.scripts
...
const createPublishAndGenerate =
  ({
    cwd,
    moduleDirAbs,
    moduleName,
...
const startWatching = (moduleDirAbs: string, onChange: () => void): FSWatcher =>
  watch(moduleDirAbs, { recursive: true }, (_eventType, filename) => {
    if (typeof filename !== 'string') return
    if (!filename.endsWith('.ts')) return
    onChange()
  })
const startDevServer = (cwd: string, pkg: null | Record<string, unknown>): ChildProcess => {
...
const dev = async (args: string[] = []) => {
  ...
  const packageJsonPath = findPackageJsonFile(cwd)
  if (!packageJsonPath) {
  ...
  let watcherTimer: null | ReturnType<typeof setTimeout> = null
  ...
  const devProcess = startDevServer(cwd, packageJson)
