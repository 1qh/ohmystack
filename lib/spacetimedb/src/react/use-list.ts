 ...
 */
const useList = <T extends Rec>(data: readonly T[], isReady: boolean, options?: 'skip' | UseListOptions<T>) => {
  const skipped = options === 'skip'
  const opts = skipped ? undefined : options
  const pageSize = opts?.pageSize ?? DEFAULT_PAGE_SIZE
  const rawQuery = opts?.search?.query ?? ''
  const debounceMs = opts?.search?.debounceMs
  const [debouncedQuery, setDebouncedQuery] = useState(rawQuery)
  const [currentPage, setCurrentPage] = useState(opts?.page ?? 1)
  const whereRef = useRef(opts?.where)
  const searchQueryRef = useRef(rawQuery)
  useEffect(() => {
    if (!debounceMs) {
  ...
  const sorted = useMemo(() => (skipped ? [] : sortData(searched, opts?.sort)), [searched, opts?.sort, skipped])
  const totalCount = sorted.length
  const cappedPageSize = Math.max(1, pageSize)
  const visibleCount = currentPage * cappedPageSize
  const pagedData = useMemo(() => sorted.slice(0, visibleCount), [sorted, visibleCount])
