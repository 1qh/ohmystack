...
const toStringField = (obj: Record<string, unknown>, key: string): string | undefined => {
  const value = obj[key]
  return typeof value === 'string' ? value : undefined
}
const toMethodField = (obj: Record<string, unknown>): 'POST' | 'PUT' | undefined => {
  const value = obj.method
  return value === 'POST' || value === 'PUT' ? value : undefined
}
const toHeadersField = (obj: Record<string, unknown>): Record<string, string> => {
...
const parsePresignedPayload = (payload: unknown): PresignedUpload => {
  if (!(typeof payload === 'object' && payload !== null))
    return err('VALIDATION_FAILED', { message: 'Invalid upload URL payload' })
  const obj = payload as Record<string, unknown>
  const uploadUrl = toStringField(obj, 'uploadUrl')
  const storageKey = toStringField(obj, 'storageKey')
...
const requestPresignedUrl = async (apiEndpoint: string, file: File): Promise<PresignedUpload> => {
...
const registerAbortListener = (xhr: XMLHttpRequest, signal?: AbortSignal): boolean => {
...
const addProgressListener = (xhr: XMLHttpRequest, onProgress: (progress: number) => void) => {
...
const uploadWithXhr = async ({
  ...
  new Promise(resolve => {
    ...
    const handlers = makeUploadHandlers(xhr, presigned, resolve)
    const isAborted = startXhrUpload({ file, presigned, signal, xhr })
...
const useUpload = (config?: UploadConfig) => {
  ...
  const register = async (file: File, storageKey: string): Promise<RegisteredFile | undefined> => {
  ...
  const setUploadProgress = (uploadId: number, value: number) => {
    if (uploadId === latestUploadIdRef.current) setProgress(value)
  }
  const onUploadError = (code: UploadErrorCode) => {
    setError(code)
  }
  const completeUpload = (uploadId: number, storageId: string, nextUrl?: string): UploadResult => {
    if (nextUrl) setUrl(nextUrl)
    if (uploadId === latestUploadIdRef.current) setProgress(100)
    return { ok: true, storageId, url: nextUrl }
  }
  const runUpload = async (file: File, uploadId: number, signal?: AbortSignal): Promise<UploadResult> => {
