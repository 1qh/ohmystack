...
type UpdateArgs<F extends OrgCrudFieldBuilders, Id> = Partial<
	OrgCrudFieldValues<F>
> & {
...
const checkMembership = <OrgId, Member extends OrgCrudMemberLike<OrgId>>(
		...
		for (const member of orgMemberTable)
			if (
				...
				identityEquals(member.userId, sender)
			)
				return member;
		return null;
	},
	...
	>(
		...
		const {
			...
			requireMembershipOrOwner = ({
				...
				if (isOrgOwnerFn?.(db, orgId, sender))
					return { isAdmin: true, orgId, userId: sender } as unknown as Member;
		...
		for (const key of optionalKeys) {
			...
			if (field)
				updateParams[key] = field as TypeBuilder<unknown, AlgebraicTypeType>;
		}
		...
		for (const key of fieldKeys)
			createParams[key] = fields[key] as TypeBuilder<
				unknown,
				AlgebraicTypeType
			>;
		const createReducer = spacetimedb.reducer(
				...
				(ctx, args) => {
					if (options?.rateLimit)
						enforceRateLimit(tableName, ctx.sender, options.rateLimit);
					...
					const { orgId, ...payload } = data as unknown as Record<
							string,
							unknown
						> & { orgId: OrgId },
						row = table.insert({
				...
				(ctx, args) => {
					const typedArgs = args as UpdateArgs<F, Id>,
						...
						{ pk, row } = getOrgOwnedRow({
							id: typedArgs.id,
							isOwner: isOrgOwnerFn
								? (oid: unknown) =>
										isOrgOwnerFn(ctx.db, oid as OrgId, ctx.sender)
								: undefined,
							operation: "update",
							orgMemberTable,
							...
							sender: ctx.sender,
							table: table as unknown as OrgCrudTableLike<
								OrgCrudOwnedRow<unknown>
							>,
							tableName,
						});
					if (
						typedArgs.expectedUpdatedAt !== undefined &&
						!timestampEquals(row.updatedAt, typedArgs.expectedUpdatedAt)
					)
					...
					const prev = row as unknown as Row,
						next = pk.update(
							applyPatch(prev, patch, ctx.timestamp),
						) as unknown as Row;
