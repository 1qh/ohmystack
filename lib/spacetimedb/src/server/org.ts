...
const findOrgBySlug = <OrgRow extends { slug: string }>(slugIndex: Iterable<OrgRow>, slug: string): null | OrgRow => {
...
const requireRole = <OrgId, MemberId>({
  minRole,
  operation,
  org,
  orgMemberTable,
  ...
  const role = getRole(org, member, sender)
...
const applyOrgUpdate = <OrgId, MemberId, OrgRow extends OrgRowLike<OrgId> & { slug: string }>(opts: {
  ...
  const nextRecord = {
    ...(opts.org as unknown as Record<string, unknown>),
    updatedAt: opts.timestamp
  } as Record<string, unknown>
  const argKeys = Object.keys(opts.args)
...
const removeByPk = <Id>(rows: Iterable<{ id: Id }>, pk: { delete: (id: Id) => boolean }, message: string) => {
  for (const row of rows) {
    const removed = pk.delete(row.id)
    if (!removed) throw makeError('NOT_FOUND', message)
  }
}
const removeCascadeRows = <DB, OrgId>(
  cascadeTables: CascadeTableConfig<DB, OrgId>[] | undefined,
  db: DB,
  orgId: OrgId
) => {
...
const removeMembersByOrg = <OrgId, MemberRow>(
...
const mergeReducerExports = (...parts: OrgExports[]): OrgExports => {
...
>(
  ...
  const optionalOrgFields = makeOptionalFields(orgFields)
  const updateParams: Record<string, TypeBuilder<unknown, AlgebraicTypeType>> = {
    orgId: config.builders.orgId
  }
  const optionalKeys = Object.keys(optionalOrgFields)
  ...
  const createReducer = spacetimedb.reducer(
    ...
    (ctx, args: Record<string, unknown>) => {
      ...
      const orgSlugIndex = config.orgSlugIndex(orgTable)
      const { slug } = args
      ...
      const inserted = orgTable.insert(payload)
      const orgMemberTable = config.orgMemberTable(ctx.db)
  ...
  const updateReducer = spacetimedb.reducer({ name: 'org_update' }, updateParams, (ctx, args) => {
    const typedArgs = args as Record<string, unknown> & { orgId: OrgId }
    const orgTable = config.orgTable(ctx.db)
    const orgPk: OrgPkLike<OrgRow, OrgId> = config.orgPk(orgTable)
    const orgMemberTable = config.orgMemberTable(ctx.db)
    const orgSlugIndex: OrgSlugIndexLike<OrgRow> = config.orgSlugIndex(orgTable)
    const org = orgPk.find(typedArgs.orgId)
  ...
  const removeReducer = spacetimedb.reducer({ name: 'org_remove' }, { orgId: config.builders.orgId }, (ctx, args) => {
    const typedArgs = args as { orgId: OrgId }
    const orgTable = config.orgTable(ctx.db)
    const orgPk: OrgPkLike<OrgRow, OrgId> = config.orgPk(orgTable)
    const orgMemberTable = config.orgMemberTable(ctx.db)
    const orgInviteTable = config.orgInviteTable(ctx.db)
    const orgInvitePk: OrgInvitePkLike<InviteRow, InviteId> = config.orgInvitePk(orgInviteTable)
    const orgJoinRequestTable = config.orgJoinRequestTable(ctx.db)
    const orgJoinRequestPk: OrgJoinRequestPkLike<JoinRequestRow, RequestId> = config.orgJoinRequestPk(orgJoinRequestTable)
    const org = orgPk.find(typedArgs.orgId)
    ...
    const inviteByOrg = config.orgInviteByOrgIndex(orgInviteTable)
    const memberByOrg = config.orgMemberByOrgIndex(orgMemberTable)
  ...
  const memberReducers = makeMemberReducers(spacetimedb, {
    builders: {
  ...
  const inviteReducers = makeInviteReducers(spacetimedb, {
    builders: {
  ...
  const joinReducers = makeJoinReducers(spacetimedb, {
    builders: {
      isAdmin: config.builders.isAdmin,
  ...
  const lifecycleReducers: OrgExports = {
    exports: {
...
const wrapByOrgIndex = <Row, OrgId>(
...
>(tables: {
  org: (db: DB) => Iterable<OrgRow> & {
    id: {
      ...
      find: (id: OrgId) => null | OrgRow
      update: (row: OrgRow) => OrgRow
    }
  ...
  orgInvite: (db: DB) => OrgInviteTableLike<InviteRow> & {
    id: {
      delete: (id: InviteId) => boolean
      find: (id: InviteId) => InviteRow | null
    }
  ...
  | 'orgTable'
> => ({
  orgByUserIndex: tbl => tbl,
  orgInviteByOrgIndex: tbl =>
    wrapByOrgIndex(
      asRec(tbl) as unknown as Iterable<InviteRow> & {
        orgId: { filter: (orgId: OrgId) => Iterable<InviteRow> }
      }
    ),
