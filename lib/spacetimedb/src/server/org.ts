...
> {
	...
	) => OrgInviteByOrgIndexLike<InviteRow, OrgId>;
	...
	orgJoinRequestPk: (
		table: OrgJoinRequestTableLike<JoinRequestRow>,
	) => OrgJoinRequestPkLike<JoinRequestRow, RequestId>;
	orgJoinRequestTable: (db: DB) => OrgJoinRequestTableLike<JoinRequestRow>;
	orgMemberByOrgIndex: (
		table: OrgMemberTableLike<MemberRow>,
	) => OrgMemberByOrgIndexLike<MemberRow, OrgId>;
	orgMemberByUserIndex: (
		table: OrgMemberTableLike<MemberRow>,
	) => Iterable<MemberRow>;
	orgMemberPk: (
		table: OrgMemberTableLike<MemberRow>,
	) => OrgMemberPkLike<MemberRow, MemberId>;
...
const makeOptionalFields = (fields: OrgFieldBuilders) => {
	...
	findOrgBySlug = <OrgRow extends { slug: string }>(
		slugIndex: Iterable<OrgRow>,
		slug: string,
	): null | OrgRow => {
	...
	applyOrgUpdate = <
		OrgId,
		MemberId,
		OrgRow extends OrgRowLike<OrgId> & { slug: string },
	>(opts: {
	...
	>(
		...
		const createReducer = spacetimedb.reducer(
				...
				(ctx, args: Record<string, unknown>) => {
					...
					const existing = findOrgBySlug(
						orgSlugIndex as Iterable<OrgRow>,
						slug,
					);
				...
				(ctx, args) => {
					const typedArgs = args as { orgId: OrgId },
						...
						orgInvitePk: OrgInvitePkLike<InviteRow, InviteId> =
							config.orgInvitePk(orgInviteTable),
					...
					const joinByOrg =
							config.orgJoinRequestByOrgIndex(orgJoinRequestTable),
						inviteByOrg = config.orgInviteByOrgIndex(orgInviteTable),
						memberByOrg = config.orgMemberByOrgIndex(orgMemberTable);
					removeByPk(
						joinByOrg.filterByOrg(typedArgs.orgId),
						orgJoinRequestPk,
						"org:remove_join_request",
					);
					removeByPk(
						inviteByOrg.filterByOrg(typedArgs.orgId),
						orgInvitePk,
						"org:remove_invite",
					);
					removeMembersByOrg(memberByOrg, typedArgs.orgId, orgMemberTable);
					if (!orgPk.delete(typedArgs.orgId))
			...
			inviteReducers = makeInviteReducers(spacetimedb, {
				...
				orgJoinRequestByOrgStatusIndex:
					config.orgJoinRequestByOrgStatusIndex as unknown as (
				...
				orgJoinRequestPk: config.orgJoinRequestPk as unknown as (
					table: Iterable<JoinRequestRow>,
				) => {
					update: (row: JoinRequestRow) => JoinRequestRow;
				},
				orgJoinRequestTable: config.orgJoinRequestTable as unknown as (
					db: DB,
				) => Iterable<JoinRequestRow>,
				orgMemberTable: config.orgMemberTable,
				orgPk: config.orgPk,
	...
	> => ({
		...
		orgInviteTable: tables.orgInvite as (
			db: DB,
		) => OrgInviteTableLike<InviteRow>,
		...
		orgJoinRequestByOrgStatusIndex: (tbl) => {
			...
			return {
				filterByOrgStatus: (orgId: OrgId, status: string) => {
					...
					for (const row of table.orgId.filter(orgId))
		...
		orgJoinRequestPk: (tbl) =>
			asRec(tbl).id as OrgJoinRequestPkLike<JoinRequestRow, RequestId>,
		orgJoinRequestTable: tables.orgJoinRequest as (
			db: DB,
		) => OrgJoinRequestTableLike<JoinRequestRow>,
		...
		orgMemberTable: tables.orgMember as (
			db: DB,
		) => OrgMemberTableLike<MemberRow>,
		orgPk: (tbl) => asRec(tbl).id as OrgPkLike<OrgRow, OrgId>,
		orgSlugIndex: (tbl) => tbl,
		orgTable: tables.org as (
			db: DB,
		) => Iterable<OrgRow> & { insert: (row: OrgRow) => OrgRow },
	});
