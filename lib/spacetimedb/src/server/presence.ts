...
const isAuthenticated = (sender: Identity): boolean => {
  ...
  const normalized = raw.trim().toLowerCase().replace(ZERO_PREFIX_REGEX, '')
  if (!normalized) return false
  for (const ch of normalized) if (ch !== '0') return true
  return false
}
const toMicros = (timestamp: Timestamp): bigint => {
  const value = timestamp as unknown as { microsSinceUnixEpoch?: bigint }
  return value.microsSinceUnixEpoch ?? 0n
}
const findPresenceRow = <Id, Row extends PresenceRow<Id>>(
  rows: Iterable<Row>,
  roomId: string,
  sender: Identity
): null | Row => {
...
const upsertPresence = <Id, Row extends PresenceRow<Id>>({
  args,
  ctx,
  pk,
  table
...
const presenceTable = <T>(presence: T): { presence: T } => ({ presence })
...
>(
  ...
  const heartbeatName = `presence_heartbeat_${tableName}`
  const leaveName = `presence_leave_${tableName}`
  const cleanupName = `presence_cleanup_${tableName}`
  const heartbeat = spacetimedb.reducer(
    { name: heartbeatName },
  ...
  const leave = spacetimedb.reducer({ name: leaveName }, { roomId: roomIdField }, (ctx, args) => {
  ...
  const cleanup = spacetimedb.reducer({ name: cleanupName }, {}, ctx => {
    const table = tableAccessor(ctx.db)
    const pk = pkAccessor(table)
    const cutoffMicros = toMicros(ctx.timestamp) - BigInt(PRESENCE_TTL_MS) * MICROS_PER_MILLISECOND
    for (const row of table.iter())
      if (toMicros(row.lastSeen) < cutoffMicros && !pk.delete(row.id)) throw makeError('NOT_FOUND', `${tableName}:cleanup`)
  })
  const exportsRecord = {
    [cleanupName]: cleanup,
