...
interface RegisterAllSchemas {
	...
	children?: Record<
		string,
		{ foreignKey: string; parent: string; schema: ZodLike }
	>;
	file?: boolean | string;
...
const isPromiseLike = (value: unknown): value is PromiseLike<unknown> => {
	...
	mergeGlobalBeforeCreate = (
		...
		return (ctx, { data: initialData }) => {
			...
			if (left.beforeCreate)
				data = requireSync(
					left.beforeCreate(ctx, { data }),
					"global.beforeCreate:left",
				);
			if (right.beforeCreate)
				data = requireSync(
					right.beforeCreate(ctx, { data }),
					"global.beforeCreate:right",
				);
			return data;
		};
	},
	mergeGlobalAfterCreate = (
		left: GlobalHooks,
		right: GlobalHooks,
	...
	mergeGlobalBeforeUpdate = (
		...
		return (ctx, { patch: initialPatch, prev }) => {
			...
			if (left.beforeUpdate)
				patch = requireSync(
					left.beforeUpdate(ctx, { patch, prev }),
					"global.beforeUpdate:left",
				);
			if (right.beforeUpdate)
				patch = requireSync(
					right.beforeUpdate(ctx, { patch, prev }),
					"global.beforeUpdate:right",
				);
			return patch;
		};
	},
	mergeGlobalAfterUpdate = (
		left: GlobalHooks,
		right: GlobalHooks,
	...
	mergeGlobalHooks = (
		left: GlobalHooks | undefined,
		right: GlobalHooks | undefined,
	...
	hasCrudHooks = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
	...
	>(
		...
		return (ctx, { data: initialData }) => {
			...
			if (localHooks?.beforeCreate)
				data = requireSync(
					localHooks.beforeCreate(ctx, { data }),
					"crud.beforeCreate:local",
				);
			return data;
		};
	},
	mergeCrudAfterCreate = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
		...
		return (ctx, { data, row }) => {
			...
			if (localHooks?.afterCreate)
				requireSync(
					localHooks.afterCreate(ctx, { data, row }),
					"crud.afterCreate:local",
				);
		};
	},
	mergeCrudBeforeUpdate = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
		...
		return (ctx, { patch: initialPatch, prev }) => {
			...
			if (localHooks?.beforeUpdate)
				patch = requireSync(
					localHooks.beforeUpdate(ctx, { patch, prev }),
					"crud.beforeUpdate:local",
				);
			return patch;
		};
	},
	mergeCrudAfterUpdate = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
		...
		return (ctx, { next, patch, prev }) => {
			...
			if (localHooks?.afterUpdate)
				requireSync(
					localHooks.afterUpdate(ctx, { next, patch, prev }),
					"crud.afterUpdate:local",
				);
		};
	},
	mergeCrudBeforeDelete = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
		...
		return (ctx, { row }) => {
			...
			if (localHooks?.beforeDelete)
				requireSync(
					localHooks.beforeDelete(ctx, { row }),
					"crud.beforeDelete:local",
				);
		};
	},
	mergeCrudAfterDelete = <
		DB,
		Row extends Rec,
		CreateArgs extends Rec,
		UpdatePatch extends Rec,
	>(
		...
		return (ctx, { row }) => {
			if (globalHooks?.afterDelete)
				requireSync(
					globalHooks.afterDelete(toGlobalCtx(table, ctx), { row: row as Rec }),
					"crud.afterDelete:global",
				);
			if (localHooks?.afterDelete)
				requireSync(
					localHooks.afterDelete(ctx, { row }),
					"crud.afterDelete:local",
				);
		};
	},
	mergeCrudHooks = <
		...
		const middlewareHooks =
			...
			crud = (factoryConfig: Parameters<typeof makeCrud>[1]) => {
				const mergedHooks = mergeCrudHooks(
						...
						globalHooks,
						factoryConfig.options?.hooks as
							| CrudHooks<unknown, Rec, Rec, Rec>
							| undefined,
					),
					...
					result = makeCrud(
						spacetimedb,
						nextConfig as Parameters<typeof makeCrud>[1],
					);
			...
			orgCrud = (factoryConfig: Parameters<typeof makeOrgCrud>[1]) => {
				const mergedHooks = mergeCrudHooks(
					...
					result = makeOrgCrud(
						spacetimedb,
						nextConfig as Parameters<typeof makeOrgCrud>[1],
					);
				registerExports(accumulatedExports, result.exports);
				return result;
			},
			childCrud = (factoryConfig: Parameters<typeof makeChildCrud>[1]) => {
				const mergedHooks = mergeCrudHooks(
					...
					nextConfig = mergedHooks
						...
						: factoryConfig,
					result = makeChildCrud(
						spacetimedb,
						nextConfig as Parameters<typeof makeChildCrud>[1],
					);
				registerExports(accumulatedExports, result.exports);
				return result;
			},
			singletonCrud = (
				factoryConfig: Parameters<typeof makeSingletonCrud>[1],
			) => {
				const mergedHooks = mergeSingletonHooks(
						factoryConfig.tableName,
						globalHooks,
						factoryConfig.options?.hooks as
							| SingletonHooks<unknown, Rec, Rec>
							| undefined,
					),
					nextConfig = mergedHooks
						? {
						...
						: factoryConfig,
					result = makeSingletonCrud(
						spacetimedb,
						nextConfig as Parameters<typeof makeSingletonCrud>[1],
					);
				registerExports(accumulatedExports, result.exports);
				return result;
			},
			...
			org = (factoryConfig: Parameters<typeof makeOrg>[1]) => {
				const result = makeOrg(
					spacetimedb as unknown as Parameters<typeof makeOrg>[0],
					factoryConfig,
				);
				registerExports(accumulatedExports, result.exports);
				return result;
...
const dbTable: (db: unknown, name: string) => unknown = (db, name) =>
		(db as Record<string, unknown>)[name],
	pkById = (tbl: unknown) => (tbl as Record<string, unknown>).id,
	pkByKey = (name: string) => (tbl: unknown) =>
	...
	makeIsOrgOwner =
		(
			orgTableAccessor: TableAccessor,
		): ((db: unknown, orgId: unknown, sender: Identity) => boolean) =>
		(db, orgId, sender) => {
			const orgTable = orgTableAccessor(db) as Iterable<{
				id: unknown;
				userId: Identity;
			}>;
			for (const org of orgTable)
				if (Object.is(org.id, orgId) && identityEquals(org.userId, sender))
					return true;
			return false;
		},
	...
	resolveCrudFields = (
		...
		isZodObject(fields) && defaults.t
			? zodToStdbFields(fields.shape, defaults.t, tableName)
			: fields;
interface RegCtx {
	defaults: CrudDefaults;
	expectedUpdatedAtField: TypeBuilder<unknown, AlgebraicTypeType>;
...
type RegTableOpts =
	| Record<
			string,
			CacheOptions & CrudOptions & OrgCrudOptions & { key?: string }
	  >
	| undefined;
...
const regOwned = (schemas: Record<string, ZodLike>, ctx: RegCtx) => {
		...
		for (const name of names) {
			...
			if (fields)
				ctx.s.crud({
					...
					fields: resolveCrudFields(
						fields,
						name,
						ctx.defaults,
					) as CrudFieldBuilders,
			...
			if (fields) {
				...
				ctx.s.cacheCrud({
					fields: resolveCrudFields(
						...
						name,
						ctx.defaults,
					) as CacheFieldBuilders,
					keyField: ctx.idField as never,
					keyName,
					options:
						tableOpts?.ttl === undefined ? undefined : { ttl: tableOpts.ttl },
	...
	regFile = (
		file: boolean | string,
		ctx: RegCtx & { spacetimedb: SpacetimeDbLike; stdbT: ZodBridgeT },
	) => {
	...
	setupCrud = (
		...
		const s = setup(spacetimedb, config),
			resolvedDefaults: Required<CrudDefaults> = {
				expectedUpdatedAtField:
					defaults.expectedUpdatedAtField ?? t.timestamp(),
				foreignKeyField:
					defaults.foreignKeyField ?? defaults.idField ?? t.u32(),
		...
		return {
			...
			cacheCrud: (
				...
				const resolvedFields = resolveCrudFields(
					fields,
					tableName,
					resolvedDefaults,
				);
			...
			m: (
				...
				const reducer = spacetimedb.reducer(
					{ name },
					params as never,
					(ctxRaw: unknown, args: unknown) => {
						const ctx = ctxRaw as {
							db: unknown;
							sender?: Identity;
							timestamp: Timestamp;
						...
						handler(
							{ db: ctx.db, sender: ctx.sender, timestamp: ctx.timestamp },
							args,
						);
					},
			...
			org: (
				...
				const orgTypes = orgOpts?.t ?? stdbT,
					resolvedOrgFields = resolveCrudFields(
						orgFields,
						"org",
						resolvedDefaults,
					) as OrgFieldBuilders,
			...
			orgCrud: (
				...
				return s.orgCrud({
					...
					orgMemberTable: (options?.orgMemberTable ??
						tblOf("orgMember")) as never,
					pk: pkById as never,
					table: tblOf(tableName) as never,
					tableName,
				});
			...
			singletonCrud: (
				tableName: string,
				fields: SingletonFieldBuilders | ZodLike,
...
type ZodKeys<F> = F extends { shape: infer S extends Record<string, unknown> }
	? keyof S & string
	: string;
const compoundIndexToEntry = (
		columns: string[],
	): { accessor: string; algorithm: "btree"; columns: string[] } => ({
		accessor: columns
	...
	bsZod = (fields: unknown): undefined | ZodLike =>
		isZodObject(fields) ? fields : undefined,
	oKeys = (obj: object): string[] => Object.keys(obj),
	withPubIndex = (
		index: string[] | undefined,
		pub: boolean | string | undefined,
	): string[] | undefined => {
	...
	applyMod = (field: FieldBuilder, mod: "index" | "unique"): FieldBuilder => {
		...
		if (typeof record[mod] === "function")
			return (record[mod] as () => FieldBuilder)();
		return field;
	},
	applyModFields = (
	...
	mergeModifierExtra = (
		...
		if (zod?.shape) {
			...
			if (indexFields && applyModFields(resolved, indexFields, "index", result))
				hasFields = true;
			if (
				uniqueFields &&
				applyModFields(resolved, uniqueFields, "unique", result)
			)
				hasFields = true;
		}
		if (extra) {
	...
	collectBsSchema = (
		...
		m: BsTag,
		ctx: BsCtx,
	): { fileNs?: boolean | string; orgZod?: ZodLike } => {
	...
	makeBsHelpers = (raw: SchemaHelpers) => {
		const cacheTable = (
				keyFieldOrName: string | TblKey,
				fields: TblInput,
				options?: { ttl?: number },
			): BsTable => {
				const keyName =
					typeof keyFieldOrName === "string"
						? keyFieldOrName
						: keyFieldOrName.name;
				return bsOf(
			...
			orgScopedTable = <F extends TblInput>(
				fields: F,
				options?: OrgScopedOpts<F>,
			): BsTable => {
				const {
						cascade = true,
						compoundIndex,
					...
					sdExtra = softDelete
						? { ...extra, deletedAt: raw.t.timestamp().optional() }
						: extra,
					...
					resolvedIndexes = compoundIndex
						? [compoundIndexToEntry(compoundIndex), ...(indexes ?? [])]
						: indexes,
			...
			orgTable = <F extends TblInput>(
				fields: F,
				options?: OrgTableOpts<F>,
			): BsTable => {
				...
				return bsOf(
					{ category: "org", zod: bsZod(fields) },
					raw.ownedTable(fields, mergedExtra),
				);
			},
			ownedTable = <F extends TblInput>(
				fields: F,
				options?: OwnedOpts<F>,
			): BsTable => {
				const {
			...
			singletonTable = (fields: TblInput): BsTable =>
				bsOf(
					{ category: "singleton", zod: bsZod(fields) },
					raw.singletonTable(fields),
				),
			tableBase = <F extends TableArgInput>(...args: TableArgs<F>): BsTable => {
				...
				if (brand === "owned")
					return ownedTable(
						fields as OwnedBranded,
						options as OwnedOpts<OwnedBranded>,
					);
				if (brand === "org")
					return orgScopedTable(
						fields as OrgScopedBranded,
						options as OrgScopedOpts<OrgScopedBranded>,
					);
				if (brand === "orgDef")
					return orgTable(
						fields as OrgDefBranded,
						options as OrgTableOpts<OrgDefBranded>,
					);
				if (brand === "singleton")
					return singletonTable(fields as SingletonBranded);
				if (brand === "base") {
					if (
						!(
							options &&
						...
						return err("VALIDATION_FAILED", {
							message:
								"Base schema tables require options.key when using table()",
						});
				...
				return err("VALIDATION_FAILED", {
					message:
						"Unknown schema brand. Use makeOwned/makeOrgScoped/makeOrg/makeBase/makeSingleton before table()",
				});
			},
	...
	noboilStdb = (
		...
		for (const name of names) {
			...
			if (entry) {
				...
				const { fileNs: fn, orgZod: oz } = collectBsSchema(
					name,
					entry.__bs,
					ctx,
				);
		...
		if (orgZod)
			s.org(
				orgZod,
				ctx.cascades.length > 0 ? { cascadeTables: ctx.cascades } : undefined,
			);
		...
		for (const name of names) {
			...
			if (entry) {
				let sqls: string[];
				if (
					entry.__bs.category === "children" &&
					entry.__bs.childFk &&
					entry.__bs.childParent
				) {
		...
		if (regSym && ctxSym)
			(g[regSym] as (schemaCtx: unknown, n: string) => void)(g[ctxSym], "__bs");
