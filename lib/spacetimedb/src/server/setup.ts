...
const requireSync = <T>(value: Promise<T> | T, hookName: string): T => {
...
const mergeGlobalBeforeUpdate = (left: GlobalHooks, right: GlobalHooks): GlobalHooks['beforeUpdate'] => {
...
const mergeGlobalHooks = (left: GlobalHooks | undefined, right: GlobalHooks | undefined): GlobalHooks | undefined => {
...
const mergeCrudAfterCreate = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const mergeCrudBeforeUpdate = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const mergeCrudAfterUpdate = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const mergeCrudBeforeDelete = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const mergeCrudAfterDelete = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const mergeCrudHooks = <DB, Row extends Rec, CreateArgs extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: CrudHooks<DB, Row, CreateArgs, UpdatePatch> | undefined
...
const hasSingletonHooks = <DB, Row extends Rec, UpdatePatch extends Rec>(
...
const mergeSingletonBeforeCreate = <DB, Row extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
...
const mergeSingletonAfterCreate = <DB, Row extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: SingletonHooks<DB, Row, UpdatePatch> | undefined
...
const mergeSingletonBeforeUpdate = <DB, Row extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: SingletonHooks<DB, Row, UpdatePatch> | undefined
...
const mergeSingletonAfterUpdate = <DB, Row extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: SingletonHooks<DB, Row, UpdatePatch> | undefined
...
const mergeSingletonHooks = <DB, Row extends Rec, UpdatePatch extends Rec>(
  table: string,
  globalHooks: GlobalHooks | undefined,
  localHooks: SingletonHooks<DB, Row, UpdatePatch> | undefined
...
const registerExports = (target: ReducerExportRecord, next: ReducerExportRecord) => {
...
const setup = (spacetimedb: SpacetimeDbLike, config: SetupConfig = {}) => {
  const middlewareHooks =
    config.middleware && config.middleware.length > 0 ? composeMiddleware(...config.middleware) : undefined
  const globalHooks = mergeGlobalHooks(config.hooks, middlewareHooks)
  const accumulatedExports: ReducerExportRecord = {}
  const crud = (factoryConfig: CrudConfigLoose) => {
    const mergedHooks = mergeCrudHooks(factoryConfig.tableName, globalHooks, factoryConfig.options?.hooks)
    const nextConfig = mergedHooks
      ? {
          ...factoryConfig,
          options: {
      ...
      : factoryConfig
    const result = makeCrud(spacetimedb, nextConfig as Parameters<typeof makeCrud>[1])
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const orgCrud = (factoryConfig: OrgCrudConfigLoose) => {
    const mergedHooks = mergeCrudHooks(factoryConfig.tableName, globalHooks, factoryConfig.options?.hooks)
    const nextConfig = mergedHooks
      ? {
          ...factoryConfig,
      ...
      : factoryConfig
    const result = makeOrgCrud(spacetimedb, nextConfig as Parameters<typeof makeOrgCrud>[1])
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const childCrud = (factoryConfig: ChildCrudConfigLoose) => {
    const mergedHooks = mergeCrudHooks(factoryConfig.tableName, globalHooks, factoryConfig.options?.hooks)
    const nextConfig = mergedHooks
      ? {
          ...factoryConfig,
      ...
      : factoryConfig
    const result = makeChildCrud(spacetimedb, nextConfig as Parameters<typeof makeChildCrud>[1])
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const singletonCrud = (factoryConfig: SingletonConfigLoose) => {
    ...
    const nextConfig = mergedHooks
      ...
      : factoryConfig
    const result = makeSingletonCrud(spacetimedb, nextConfig as Parameters<typeof makeSingletonCrud>[1])
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const cacheCrud = (factoryConfig: CacheConfigLoose) => {
    const result = makeCacheCrud(spacetimedb, factoryConfig as Parameters<typeof makeCacheCrud>[1])
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const org = (factoryConfig: Parameters<typeof makeOrg>[1]) => {
    const result = makeOrg(spacetimedb as unknown as Parameters<typeof makeOrg>[0], factoryConfig)
    registerExports(accumulatedExports, result.exports)
    return result
  }
  const allExports = (): ReducerExportRecord => ({ ...accumulatedExports })
  return {
    allExports,
    cacheCrud,
    childCrud,
...
const pkById = (tbl: unknown) => (tbl as Record<string, unknown>).id
const pkByKey = (name: string) => (tbl: unknown) => (tbl as Record<string, unknown>)[name]
const tblOf =
  (name: string): TableAccessor =>
  db =>
    dbTable(db, name)
const makeIsOrgOwner =
  (orgTableAccessor: TableAccessor): ((db: unknown, orgId: unknown, sender: Identity) => boolean) =>
  (db, orgId, sender) => {
    const orgTable = orgTableAccessor(db) as Iterable<{
      id: unknown
      userId: Identity
    }>
    for (const org of orgTable) if (Object.is(org.id, orgId) && identityEquals(org.userId, sender)) return true
    return false
  }
const isZodObject = (v: unknown): v is ZodLike =>
  typeof v === 'object' &&
  v !== null &&
  'type' in v &&
...
const resolveCrudFields = (fields: unknown, tableName: string, defaults: CrudDefaults): unknown =>
...
const regOwned = (schemas: Record<string, ZodLike>, ctx: RegCtx) => {
  ...
  for (const name of names) {
    ...
    if (fields) {
      ...
      const extra = ctx.extraFieldsByTable[name]
      const merged = extra ? { ...resolved, ...extra } : resolved
      ctx.s.crud({
...
const regOrgScoped = (schemas: Record<string, ZodLike>, ctx: RegCtx) => {
  const names = Object.keys(schemas)
  for (const name of names) {
    const fields = schemas[name]
    if (fields) {
...
const regSingleton = (schemas: Record<string, ZodLike>, ctx: RegCtx) => {
  const names = Object.keys(schemas)
  for (const name of names) {
    const fields = schemas[name]
    if (fields)
...
const regBase = (schemas: Record<string, ZodLike>, ctx: RegCtx) => {
  const names = Object.keys(schemas)
  for (const name of names) {
    const fields = schemas[name]
    if (fields) {
      const tableOpts = ctx.opts?.[name]
      const keyName = tableOpts?.key ?? 'id'
      ctx.s.cacheCrud({
...
const regChildren = (schemas: Record<string, { foreignKey: string; parent: string; schema: ZodLike }>, ctx: RegCtx) => {
  const names = Object.keys(schemas)
  for (const name of names) {
...
const regFile = (file: boolean | string, ctx: RegCtx & { spacetimedb: SpacetimeDbLike; stdbT: ZodBridgeT }) => {
...
const setupCrud = (spacetimedb: SpacetimeDbLike, defaults: CrudDefaults = {}, config?: SetupConfig) => {
  const s = setup(spacetimedb, config)
  const resolvedDefaults: Required<CrudDefaults> = {
  ...
  const { expectedUpdatedAtField, idField } = resolvedDefaults
  const fkField = resolvedDefaults.foreignKeyField
  const oIdField = resolvedDefaults.orgIdField
  const stdbT = resolvedDefaults.t
  return {
    ...
    org: (
      ...
      const resolvedOrgFields = resolveCrudFields(orgFields, 'org', resolvedDefaults) as OrgFieldBuilders
...
const fkSuffix = /Id$/u
const isChildObj = (v: unknown): v is ChildLike =>
  typeof v === 'object' && v !== null && 'foreignKey' in v && 'parent' in v && 'schema' in v
const readSchemaBrand = (v: unknown): RuntimeSchemaBrand | undefined => {
...
const bsOf = (meta: BsTag, table: unknown): BsTable => ({ __bs: meta, table })
const bsZod = (fields: unknown): undefined | ZodLike => (isZodObject(fields) ? fields : undefined)
const oKeys = (obj: object): string[] => Object.keys(obj)
const withPubIndex = (index: string[] | undefined, pub: boolean | string | undefined): string[] | undefined => {
  if (typeof pub !== 'string') return index
  if (!index) return [pub]
  for (const i of index) if (i === pub) return index
  return [...index, pub]
}
const applyMod = (field: FieldBuilder, mod: 'index' | 'unique'): FieldBuilder => {
  const record = field as unknown as Record<string, unknown>
  if (typeof record[mod] === 'function') return (record[mod] as () => FieldBuilder)()
  return field
}
const applyModFields = (
...
const mergeModifierExtra = (
  ...
  const hasIndex = indexFields ? indexFields.length > 0 : false
  const hasUnique = uniqueFields ? uniqueFields.length > 0 : false
  const hasModifiers = hasIndex ? true : hasUnique
  const zod = hasModifiers ? bsZod(fields) : undefined
  let hasFields = false
  if (zod?.shape) {
...
const collectBsOpts = (name: string, m: BsTag, ctx: BsCtx) => {
...
const buildBsSchemas = (ctx: BsCtx): RegisterAllSchemas => {
...
const makeBsHelpers = (raw: SchemaHelpers) => {
  ...
  const childTable = (fkOrChild: ChildLike | string, schema?: TblChild): BsTable => {
  ...
  const orgScopedTable = <F extends TblInput>(fields: F, options?: OrgScopedOpts<F>): BsTable => {
    ...
    const resolvedIndexes = compoundIndex ? [compoundIndexToEntry(compoundIndex), ...(indexes ?? [])] : indexes
    const stdbOpts = resolvedIndexes ? { indexes: resolvedIndexes } : undefined
    return bsOf(
      {
        cascade,
        cascadeTo,
  ...
  const orgTable = <F extends TblInput>(fields: F, options?: OrgTableOpts<F>): BsTable => {
  ...
  const ownedTable = <F extends TblInput>(fields: F, options?: OwnedOpts<F>): BsTable => {
...
const noboil = (
  ...
  let fileNs: boolean | string = false
  ...
  const g = group as unknown as Record<symbol, unknown>
  const syms = Object.getOwnPropertySymbols(group)
