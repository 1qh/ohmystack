...
>(
  ...
  const getName = `get_${tableName}`
  const upsertName = `upsert_${tableName}`
  const upsertParams: SingletonFieldBuilders = {}
  ...
  const upsertReducer = spacetimedb.reducer({ name: upsertName }, upsertParams, (ctx, args) => {
    ...
    const table = tableAccessor(ctx.db)
    const existing = findByUser(table as unknown as SingletonTableLike<SingletonRow>, ctx.sender)
