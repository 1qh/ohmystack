	...
	>(
		...
		const getReducer = spacetimedb.reducer({ name: getName }, {}, (ctx) => {
				const table = tableAccessor(ctx.db),
					row = findByUser(
						table as unknown as SingletonTableLike<SingletonRow>,
						ctx.sender,
					);
					...
					if (existing) {
						if (hooks?.beforeUpdate)
							...
							hooks.beforeUpdate(hookCtx, {
								patch: patchRecord as unknown as Partial<
									SingletonFieldValues<F>
								>,
						...
						if (hooks?.beforeCreate)
							...
							hooks.beforeCreate(hookCtx, {
								data: patchRecord as unknown as Partial<
									SingletonFieldValues<F>
								>,
							});
						const newRow = {
