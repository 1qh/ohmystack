...
interface ZodBridgeT {
  ...
  u32: () => { autoInc: () => { primaryKey: () => FieldBuilder }; index: () => FieldBuilder; unique: () => FieldBuilder }
}
interface ZodLike {
  def?: {
    element?: unknown
...
const isRecord = (value: unknown): value is Record<string, unknown> => typeof value === 'object' && value !== null,
  ...
  fieldFromSchema = (schema: unknown, t: ZodBridgeT, path: string): FieldBuilder => {
    ...
    if (type === 'object')
      return t.object(
        toPascalCase(path),
        objectFields(z.def?.shape, t, { fromSchema: fieldFromSchema, prefix: path }) as Record<string, StdbTypeBuilder>
      )
    if (type === 'union') return unionObject(z.def?.options, t, { fromSchema: fieldFromSchema, path })
