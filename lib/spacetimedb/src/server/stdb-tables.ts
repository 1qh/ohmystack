...
interface ZodBridgeT {
	array: (
		element: StdbTypeBuilder,
	) => FieldBuilder & { optional: () => FieldBuilder };
	...
	object: (
		name: string,
		fields: Record<string, StdbTypeBuilder>,
	) => FieldBuilder & { optional: () => FieldBuilder };
...
const isRecord = (value: unknown): value is Record<string, unknown> =>
	...
	hasOptional = (
		value: FieldBuilder,
	): value is FieldBuilder & { optional: () => FieldBuilder } =>
		isRecord(value) &&
		"optional" in value &&
	...
	toPascalCase = (value: string): string => {
		...
		for (let i = 0; i < value.length; i += 1) {
			const ch = value.charAt(i),
				...
				splitBeforeUpper =
					isUpper && word.length > 0 && (prevIsLower || prevIsDigit);
			if (!isAlphaNum) {
				if (word.length > 0) {
	...
	asOptional = (field: FieldBuilder): FieldBuilder =>
		hasOptional(field) ? field.optional() : field,
	unwrapOptional = (z: ZodLike): undefined | ZodLike => {
		let inner = z.def?.innerType as undefined | ZodLike;
		while (inner) {
			const it =
				isRecord(inner) && typeof inner.type === "string" ? inner.type : "";
			if (it === "optional" || it === "nullable")
				inner = inner.def?.innerType as undefined | ZodLike;
			else break;
		}
		return inner;
	},
	...
	unionObject = (
		...
		for (const variant of variants) {
			...
			for (const name of names) {
				...
				if (!(name in firstSchemaByField))
					firstSchemaByField[name] = variant[name];
			}
		}
		const merged: Record<string, FieldBuilder> = {};
		fieldNames.sort((a, b) => (a < b ? -1 : a > b ? 1 : 0));
		for (const name of fieldNames) {
			const baseField = fromSchema(
				firstSchemaByField[name],
				t,
				`${path}.${name}`,
			);
			merged[name] =
				(fieldCounts[name] ?? 0) === totalVariants
					? baseField
					: asOptional(baseField);
		}
		return t.object(
			toPascalCase(path),
			merged as Record<string, StdbTypeBuilder>,
		);
	},
	objectFields = (
	...
	fieldFromSchema = (
		...
		if (type === "array")
			return t.array(
				fieldFromSchema(z.def?.element, t, `${path}.item`) as StdbTypeBuilder,
			);
		if (type === "object")
			return t.object(
				toPascalCase(path),
		...
		if (type === "optional" || type === "nullable")
			return asOptional(fieldFromSchema(unwrapOptional(z), t, path));
		if (
			type === "default" ||
			type === "readonly" ||
			type === "catch" ||
	...
	resolveFields = (
		...
		if (isZodObject(fields))
			return zodToStdbFields(fields.shape ?? {}, t, tableName);
	...
	makeSchema = (deps?: Partial<StdbDeps>) => {
		const t = deps?.t ?? (stdbT as unknown as ZodBridgeT),
			...
			cacheTable = (
				keyFieldOrName: KeyField | string,
				fields: TableInput,
				opts?: TableOptions,
			): StdbTable => {
				const keyField: KeyField =
					typeof keyFieldOrName === "string"
						? { builder: t.u32().unique(), name: keyFieldOrName }
						: keyFieldOrName;
				return tbl(
					{ ...opts },
					{
