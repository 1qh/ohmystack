...
const getReducerParamMap = (schema: SchemaResponse): Map<string, string[]> => {
...
const getSchema = async (ctx: Pick<TestContext, 'baseHttpUrl' | 'moduleName'>): Promise<SchemaResponse> => {
...
const normalizeReducerArgs = (ctx: TestContext, reducerName: string, args?: unknown): unknown[] => {
...
const getSqlFields = (schema: unknown): string[] => {
  ...
  const directElements = schemaRecord.elements
  ...
  for (const item of elementsSource)
    if (item && typeof item === 'object') {
      ...
      const nameValue = itemRecord.name
...
const rowToObject = (row: unknown, fields: string[]): unknown => {
  if (!Array.isArray(row) || fields.length === 0 || fields.length !== row.length) return row
  const result: Record<string, unknown> = {}
  const rowValues: readonly unknown[] = row
...
const postSql = async (ctx: TestContext, query: string, token: string): Promise<SqlStatementResult[]> => {
  ...
  const parsed = await parseJsonResponse<SqlStatementResult | SqlStatementResult[]>(response)
  if (Array.isArray(parsed)) return parsed
  return [parsed]
}
const postReducer = async (
  ctx: TestContext,
  request: { args: unknown[]; reducerName: string; token: string }
): Promise<unknown> => {
  const safeReducer = ensureIdentifier(request.reducerName, 'REDUCER_NAME')
  const response = await fetch(`${ctx.baseHttpUrl}/v1/database/${ctx.moduleName}/call/${safeReducer}`, {
...
const createTestContext = async (options?: CreateTestContextOptions): Promise<TestContext> => {
  ...
  const moduleName = resolveModuleName(options?.moduleName)
  const userCount = options?.userCount ?? 1
  const resolvedModuleName = ensureModuleName(moduleName)
  const defaultUser = await createConnectedUser({ baseWsUrl, moduleName: resolvedModuleName })
...
const queryTable = async (ctx: TestContext, tableName: string, user?: TestUser): Promise<unknown[]> => {
  const activeUser = user ?? ctx.defaultUser
  const safeTableName = ensureIdentifier(tableName, 'TABLE_NAME')
  const sql = `SELECT * FROM ${safeTableName}`
  const results = await postSql(ctx, sql, activeUser.token)
...
const cleanup = async (ctx: TestContext): Promise<void> => {
