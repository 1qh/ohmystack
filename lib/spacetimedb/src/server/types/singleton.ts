...
> {
	afterCreate?: (
		ctx: SingletonHookCtx<DB>,
		args: { data: UpdatePatch; row: Row },
	) => Promise<void> | void;
	afterUpdate?: (
		ctx: SingletonHookCtx<DB>,
		args: { next: Row; patch: UpdatePatch; prev: Row },
	) => Promise<void> | void;
	beforeCreate?: (
		ctx: SingletonHookCtx<DB>,
		args: { data: UpdatePatch },
	) => Promise<UpdatePatch> | UpdatePatch;
	beforeRead?: (
		ctx: SingletonHookCtx<DB>,
		args: { row: Row },
	) => Promise<void> | void;
	beforeUpdate?: (
		ctx: SingletonHookCtx<DB>,
		args: { patch: UpdatePatch; prev: Row },
	) => Promise<UpdatePatch> | UpdatePatch;
}
