...
import { callReducer, cleanup, createTestContext, queryTable } from '@ohmystack/spacetimedb/server'
...
const sleep = async (ms: number) =>
  ...
  run = async () => {
    ...
    const ctx = await createTestContext({ moduleName: '@ohmystack/spacetimedb', userCount: 2 }),
      [, user2] = ctx.users
