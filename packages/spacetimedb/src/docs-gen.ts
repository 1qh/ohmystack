...
const dim = (s: string) => `\u001B[2m${s}\u001B[0m`,
  ...
  generateMarkdown = (calls: FactoryCall[], tableFields: Map<string, { name: string; type: string }[]>): string => {
    const lines: string[] = [
      '# API Reference',
      '',
       '*Auto-generated by `ohmystack-stdb docs`*',
      '',
  ...
  extractSignature = (fileContent: string, symbolName: string): string => {
   ...
   ENTRY_POINTS: { label: string; path: string }[] = [
     { label: '@ohmystack/spacetimedb', path: 'index.ts' },
     { label: '@ohmystack/spacetimedb/schema', path: 'schema.ts' },
     { label: '@ohmystack/spacetimedb/react', path: 'react/index.ts' },
     { label: '@ohmystack/spacetimedb/server', path: 'server/index.ts' },
     { label: '@ohmystack/spacetimedb/components', path: 'components/index.ts' },
     { label: '@ohmystack/spacetimedb/next', path: 'next/index.ts' },
     { label: '@ohmystack/spacetimedb/zod', path: 'zod.ts' },
     { label: '@ohmystack/spacetimedb/seed', path: 'seed.ts' },
     { label: '@ohmystack/spacetimedb/retry', path: 'retry.ts' },
     { label: '@ohmystack/spacetimedb/eslint', path: 'eslint.ts' },
     { label: '@ohmystack/spacetimedb/test', path: 'server/test.ts' }
   ],
  processEntryPoint = (ep: { label: string; path: string }, srcDir: string, lines: string[]): number => {
   ...
   generateFullReference = (srcDir: string): string => {
     const lines: string[] = ['# @ohmystack/spacetimedb — Full API Reference', '', '*Auto-generated by `ohmystack-stdb docs --full`*', '']
    let totalSymbols = 0
