...
const lineBreakRegex = /\r?\n/u,
  ...
  syncCheck = ({ rootDir }: { rootDir: string }) => {
    const diff = runCapture({ cmd: ['git', 'diff', '--exit-code', '--', 'lib/rnr'], cwd: rootDir }),
      output = `${decode(diff.stdout)}${decode(diff.stderr)}`
    if (diff.exitCode !== 0) throw new Error(`lib/rnr is out of sync with sync script output:\n${output}`)
  },
  syncUpdate = async () => {
