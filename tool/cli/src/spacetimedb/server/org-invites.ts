...
const SEVEN_DAYS_MS = DAYS_PER_WEEK * DAY_HOURS * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MILLIS_PER_SECOND
const TOKEN_BASE = 36
...
const makeInviteToken = (): string => {
  ...
  while (token.length < TOKEN_LENGTH)
    token += ((tokenCounter.value * 7 + token.length * 13) % 2_176_782_336).toString(TOKEN_BASE)
...
const requireAdminRole = <OrgId, MemberId>({
  operation,
  org,
  orgMemberTable,
  sender
...
const findInviteByToken = <InviteId, OrgId, InviteRow extends OrgInviteRowLike<InviteId, OrgId>>(
...
const findPendingJoinRequest = <RequestId, OrgId, JoinRequestRow extends OrgJoinRequestRowLike<RequestId, OrgId>>(
...
>(
  ...
  const acceptInviteReducer = spacetimedb.reducer(
  ...
  const revokeInviteReducer = spacetimedb.reducer(
    ...
    (ctx, _args: Record<string, unknown>) => {
      ...
      const orgMemberTable = config.orgMemberTable(ctx.db)
      const orgInviteTable = config.orgInviteTable(ctx.db)
      const orgInvitePk = config.orgInvitePk(orgInviteTable)
      const invite = orgInvitePk.find(args.inviteId)
