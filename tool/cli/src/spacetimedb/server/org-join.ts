...
const findPendingJoinRequestByUser = <RequestId, OrgId, JoinRequestRow extends OrgJoinRequestRowLike<RequestId, OrgId>>(
...
>(
  ...
  const requestJoinReducer = spacetimedb.reducer(
    ...
    (ctx, _args: Record<string, unknown>) => {
      ...
      const joinByOrgStatusIndex = config.orgJoinRequestByOrgStatusIndex(orgJoinRequestTable)
  ...
  const approveJoinReducer = spacetimedb.reducer(
    ...
    (ctx, _args: Record<string, unknown>) => {
      ...
      const orgMemberTable = config.orgMemberTable(ctx.db)
      const orgJoinRequestTable = config.orgJoinRequestTable(ctx.db)
      const orgJoinRequestPk = config.orgJoinRequestPk(orgJoinRequestTable)
      const request = orgJoinRequestPk.find(args.requestId)
  ...
  const rejectJoinReducer = spacetimedb.reducer(
  ...
  const cancelJoinReducer = spacetimedb.reducer(
