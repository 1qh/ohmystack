...
test.describe
  .serial('Blog CRUD - Read', () => {
    ...
    test('can navigate to blog detail page', async ({ blogPage, page }) => {
      ...
      await card.locator('[data-testid="blog-card-link"]').click()
...
test.describe
  .serial('Blog CRUD - Update', () => {
    ...
    test('can navigate to edit page', async ({ blogPage, page }) => {
      ...
      await expect(card).toBeVisible({ timeout: 10_000 })
      await card.locator('[href*="/edit"]').first().click()
...
test.describe
  .serial('Blog CRUD - Delete', () => {
    ...
    test('shows delete confirmation dialog', async ({ blogPage }) => {
      ...
      await blogPage.createBlog(title, 'To be deleted')
      const card = blogPage.getBlogCards().filter({ hasText: title }).first()

      await expect(card).toBeVisible({ timeout: 10_000 })
      await card.getByTestId('delete-blog-trigger').click()
    ...
    test('can confirm delete', async ({ blogPage }) => {
      ...
      await blogPage.getBlogCards().filter({ hasText: title }).first().getByTestId('delete-blog-trigger').click()
