...
const WikiPage = () => {
	const { isAdmin, org } = useOrg(),
		...
		{ clear, handleBulkDelete, selected, toggleSelect, toggleSelectAll } =
			useBulkSelection({
				items: wikis,
				...
				rm: async (args) => {
					if (args.ids) {
						const tasks = args.ids.map(async (id) =>
							rmWiki({ id: Number(id) }),
						);
		...
		<div className="space-y-6">
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-2">
					<Button
						className={
							showDeleted ? "border-destructive/50 text-destructive" : ""
						}
								...
								<Link href={`/wiki/${w.id}`}>
									<Card className="transition-colors hover:bg-muted">
										...
										<CardContent className="flex items-center gap-2">
											...
											<Badge
												variant={
													w.status === "published" ? "default" : "secondary"
												}
											>
												{w.status}
											</Badge>
